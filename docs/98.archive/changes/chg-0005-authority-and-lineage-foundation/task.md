...
| ALF-005 | VAL-ALF-004, VAL-ALF-007 | Implement the ALF-002C atomic closure-gap execution admission correction before lifecycle closure. | platform | Done | Implemented; independent review returned `REQUIREMENTS COMPLIANT` and `QUALITY APPROVED`. | RED: after Spec 034 and its current Plan/Task closed, Spec 035 became the first unfinished original tranche and the exact-one gate incorrectly emitted `PROGRAM-LINEAGE-EXECUTION-GATE`; a premature Spec 036 pair produced two diagnostics because the valid Spec 035 pre-planning gap was also rejected. GREEN: the dependency-ready tranche accepts zero components before planning or one exact reciprocal direct-Spec pair after planning; plan-only, task-only, nonreciprocal, multiple, connected-extra, and premature-successor cases retain the stable gate diagnostic. The 344-case self-test and strict current-corpus validation pass, so ALF-004 may resume. |
| ALF-006 | VAL-ALF-003 | Align the exact production lineage projection assertion with the Spec 034 closure state. | platform | Done | Implemented in `2b07652`; independent review returned `REQUIREMENTS COMPLIANT` and `QUALITY APPROVED`. | RED: after the registry relation changed to `done`, the registry self-test failed with `production program-lineage typed projection differs` because the assertion hard-coded Specs 034-040 as `active`. GREEN: the exact projection now requires Spec 034/order 1/`done`/ADR-0017, Specs 035-040/orders 2-7/`active`/ADR-0017, and no follow-ups; the 78-case self-test and strict 430-path validation pass without weakening the assertion to a self-comparison. |
| ALF-003 | VAL-ALF-005 | Add the evidence-backed Current audit disposition overlay. | platform | Done | Added and root-fallback requirements/quality reviewed after the subagent thread cap. | The five selected observation rows remain byte-equal to `15b154d43c868c8a758e2021b1d7023f462ea0f4`; the dated five-row overlay links each disposition to its Spec/Plan/Task/validator, preserves remote/provider/live `DEFER`, routes broader work to Specs 035-040, and passes strict cross-document and Markdown-profile validation. |
| ALF-004 | VAL-ALF-001 through VAL-ALF-007 | Run full QA, independent review, and atomic lifecycle closure. | platform | Done | Spec 034, its Plan/Task, registry relation, indexes, and migration-ledger records are atomically closed; Spec 035 has no Plan/Task. | The repository-static command matrix passed except for the isolated all-files `strict-repository-quality` FIFO `Errno 95` owned by Spec 039. Review remediation produced the independently approved ALF-005 and ALF-006 corrections and aligned VAL-ALF-007 with their zero-or-one admission contract; final whole-closure review returned `REQUIREMENTS COMPLIANT` and `QUALITY APPROVED`. Rollback parent is `2b07652`; logical implementation commits are `71caf2b`, `31f4b0c`, `b8210cc`, `6ffd660`, and `2b07652`. |
...
The first original tranche whose state is neither `done` nor `archived` may
have no current execution component before planning. Once execution evidence
exists, it must be exactly one reciprocal pair whose two members link directly
to that Spec; plan-only, task-only, connected extra, or orphan nodes fail even
without their own Spec link.
...
### ALF-003 Current Audit Overlay Evidence

| Check | Result |
| --- | --- |
| Immutable observation-row assertion | PASS: RMD-005, RMD-009, RMD-011, RMD-013, and RMD-032 remain `5/5` byte-equal to observation commit `15b154d43c868c8a758e2021b1d7023f462ea0f4`. Overlay identifiers use code-form cells so they cannot be mistaken for source observation rows. |
| Disposition contract | PASS: one dated table has exactly the required `Finding`, `HEAD disposition`, `Repository evidence`, and `Retained boundary` columns and five finding rows. Each row names its current Spec, Plan, Task, validators, and retained evidence lane. |
| Follow-up routing | PASS: Specs 035-040 retain schema/transition, archive/retention, reference IA, GitHub CI/QA, compatibility, and program-closure ownership. Repository-static closure does not claim provider, remote, secret, Kubernetes, Vault, Argo CD, or live readiness. |
| `python3 scripts/validate-links-and-owners.py --root . --mode strict --body-contracts registry` | PASS: zero current-corpus cross-document diagnostics. |
| `python3 scripts/validate-markdown-profiles.py --root . --mode strict --body-contracts registry` | PASS: zero Markdown-profile diagnostics. |
| Root-fallback requirements review | `REQUIREMENTS COMPLIANT`: immutable rows, exact dispositions, owner links, retained boundaries, and two-file scope match ALF-003. |
| Root-fallback quality review | `QUALITY APPROVED`: exact follow-up links replace ambiguous grouped links; no duplicate observation row, readiness overclaim, broken cross-link, or Markdown violation remains. |

### ALF-004 Closure Evidence

| Command or review lane | Result |
| --- | --- |
| `python3 scripts/validate-document-contract-registry.py --root . --self-test` | PASS: 78 cases, including typed lineage mutations and isolated legacy migration proof. |
| `python3 scripts/validate-document-contract-registry.py --root . --mode strict` | PASS: 430 classified paths, two programs, baseline 433, new 58, uncovered 0, ambiguous 0. |
| `python3 scripts/validate-markdown-profiles.py --root . --self-test` and strict registry-body mode | PASS: self-test and current-corpus profile validation report no violation. |
| `python3 scripts/validate-links-and-owners.py --root . --self-test` and strict registry-body mode | PASS: 344 isolated cases and zero current-corpus cross-document diagnostics, including the Spec 035 pre-planning zero-component state. |
| `bash scripts/validate-repo-quality-gates.sh .` | PASS in the direct repository environment, including the exact GitOps ADD/DELETE/RETAIN base-ref check. |
| `pre-commit run --all-files` | DEFER for only `strict-repository-quality`: its isolated environment returns `Errno 95` from `os.mkfifo` in `validate-gitops-change-set.py --self-test`. Every other hook passes. Spec 039 owns the portability fix. |
| Post-FIFO component checks | PASS: Vault/ESO self-test and root validation, affected-surface self-test and 21/21 current checks, agent-role 480-case self-test and 30-adapter current checks, and roster self-test/currentness validation. The filtered aggregate fallback was not needed because the direct canonical aggregate completed. |
| Atomic-closure focused checks | PASS: strict registry, strict Markdown-profile, strict cross-document, JSON parse, changed-file Markdownlint, and `git diff --check`. |

The first independent ALF-004 requirements review returned `CHANGES REQUIRED`
because the old exact-one execution gate rejected Spec 035's valid
post-closure/pre-planning gap. ALF-005 corrected that contract and its fresh
independent reviews returned `REQUIREMENTS COMPLIANT` and `QUALITY APPROVED`.
The next closure review found that the exact production projection assertion
still expected Spec 034 to remain `active`; ALF-006 corrected the assertion in
`2b07652` and independently received the same two approval verdicts. These
verdicts authorize the state transition but do not pre-claim the fresh
whole-closure diff review that must occur before the closure commit.

The next final closure requirements review returned `REQUIREMENTS COMPLIANT`,
but its quality review returned `QUALITY CHANGES REQUIRED` because
VAL-ALF-007 still said that the dependency-ready tranche always had a current
pair. The criterion now states the approved zero-before-planning or exact-one-
pair-after-planning contract. Fresh whole-closure re-review then returned
`REQUIREMENTS COMPLIANT` and `QUALITY APPROVED`.

Logical implementation commits are `71caf2b` (registry v6), `31f4b0c`
(cross-document admission), `b8210cc` (Current audit overlay), `6ffd660`
(closure-gap correction), and `2b07652` (production projection correction).
Rollback starts from parent `2b07652`; the closure commit hash is intentionally
absent because it does not exist inside its own payload. Spec 035 remains an
active design contract with no Plan/Task pair.
Remote CI, provider runtime, secret, Kubernetes, Vault, ESO, Argo CD, and live
readiness remain `DEFER`.
...
| [ALF-003](../plans/2026-07-15-authority-and-lineage-foundation.md#task-3-normalize-the-current-audit-remediation-overlay) | Done. | The immutable five-row observation baseline, dated Current disposition overlay, exact owner/follow-up links, root-fallback review, and strict validator evidence are included in the logical ALF-003 commit. |
| [ALF-005](../plans/2026-07-15-authority-and-lineage-foundation.md#work-breakdown) | Done. | The ALF-002C correction's eight atomic closure-transition fixtures and 344-case self-test demonstrate the dependency-ready zero-or-one exact-pair contract; independent review returned `REQUIREMENTS COMPLIANT` and `QUALITY APPROVED`. |
| [ALF-006](../plans/2026-07-15-authority-and-lineage-foundation.md#work-breakdown) | Done. | The exact production projection now matches Spec 034 `done` plus Specs 035-040 `active`; registry self-test and strict validation pass, and independent review returned `REQUIREMENTS COMPLIANT` and `QUALITY APPROVED`. |
| [ALF-004](../plans/2026-07-15-authority-and-lineage-foundation.md#task-4-validate-and-close-the-authority-foundation) | Done. | Atomic lifecycle state, indexes, registry relation, migration ledger, repository-static command results, prior review-remediation context, rollback parent, logical commit list, and residual FIFO ownership are recorded without creating a Spec 035 Plan/Task. |
