...
assets. WORK-105 applies only the terminal document-form and authority portion;
the later archive and script migrations remain gated by their named work items.
...
Active
Historical AD-0011, through the [archive index](../../../README.md)
...
The read-only census re-authorized after the WORK-104 link-only body updates at
review base `a6fa1806` found exactly eight tracked legacy
architecture-requirement records: active ARD-0004 through ARD-0007, accepted
ARD-0008 and ARD-0009, and active ARD-0010 and ARD-0011. A terminal AD form
cannot be accepted by converting only ARD-0011; every active or accepted record
and every live consumer of the legacy form requires an exact disposition.
...
`docs/02.architecture/descriptions/ad-<id>-<slug>.md`. WORK-105 converted the
complete eight-record source ARD corpus one-to-one to terminal AD records, preserving every numeric
...
| ARD-0008 | accepted | AD-0008 / Historical `docs/02.architecture/descriptions/0008-workspace-document-assurance-operating-model.md`, through the [archive index](../../../README.md) |
| ARD-0009 | accepted | AD-0009 / Historical `docs/02.architecture/descriptions/0009-document-lifecycle-evidence-operating-model.md`, through the [archive index](../../../README.md) |
| ARD-0010 | active | AD-0010 / Historical `docs/02.architecture/descriptions/0010-repository-delivery-evidence-architecture.md`, through the [archive index](../../../README.md) |
| ARD-0011 | active | AD-0011 / Historical `docs/02.architecture/descriptions/0011-document-taxonomy-consolidation-architecture.md`, through the [archive index](../../../README.md) |
...
The conversion contains no ninth implicit mapping, renumbering, slug change,
or active/accepted state change.

The full conversion includes a complete tracked full-repository `git grep`
classifier for legacy-form profile, template, route, relationship, lifecycle,
...
requires both zero unconverted current legacy-form records and zero live or
unclassified legacy-form consumers. Literal matches may remain only in reviewed `retain-history`
surfaces. The legacy architecture-requirement and RFC forms then have no terminal active profile, template, route,
...
The pinned pre-WORK-105 registry exposed a human-authored API Spec profile,
Stage 03 route, template, and relationships. WORK-105 retired all four as part
of the atomic AD/Stage 99 forms cutover.
...
The current tracked corpus contains zero authored API Spec records. That
instance census was only the first retirement condition; it was not consumer
proof. WORK-105 satisfied two independent conditions before retirement:

1. the tracked authored API Spec instance census is zero; and
2. a full-repository `git grep` classifier assigns every authored-profile
   ...
   authored API Spec consumers; both counts are zero.
...
the retired authored API Spec surface,
the virtual Stage 98 change-directory aggregate, and embedded immutable
archive payloads. The retired authored API Spec template is also outside the
terminal surface. Embedded `original_artifact_id` is provenance, not
...
`05.operations/{GUIDE,POLICY,RUNBOOK,INCIDENT,POSTMORTEM}`. A historical
architecture-description payload maps to terminal type `AD`; its embedded
`original_artifact_id` MUST retain the reviewed historical type it actually
carried (for example, `ARD-####`) and MUST NOT be normalized to current
terminology.

The current 93-record cutover needs no `API-SPEC` tombstone type because its
reviewed corpus contains no authored API Spec record. If a historical authored
API Spec record is discovered later, terminal entry fails until a reviewed
...
WORK-104 performed the 82 moves and rebased the destination Plan/Task. WORK-105
created the AD route and Stage 99 core forms, atomically converted all eight
legacy ARD source records under the closed mapping, closed every live legacy
ARD consumer,
and separately gated AD-0011's archive-invariant replacement with ADR-0024
acceptance and the registry projection change. It also retired the authored API
Spec surface with zero-instance proof, complete consumer disposition, and
terminal negative fixtures, and preserved the native
...
conversion and both AD closure gates pass. The
...
This decision partially supersedes only ADR-0018's
...
- Moving or editing a Stage 98 record before the later schema-versioned ledger,
  validators, recovery proof, and bounded WORK-107 cutover.
...
- Replacing ADR-0023 in any unrelated registry lineage or rewriting either
  accepted predecessor beyond the necessary reciprocal supersession note.
...
standards-mandated Markdown representation. Converting ARD-0004 through
ARD-0011 to AD-0004 through AD-0011 keeps every lineage stable. It also makes
...
- **Retain the former architecture-requirement form as the terminal active form**:
...
- **Allocate new AD numbers during AD conversion**: rejected because it would
...
| --- | --- | --- |
| [ADR-0018](./0018-full-body-archive-record-and-retention.md) | Partially supersedes mirror-original-path and Tombstone prohibition only; preserves full-body payload, provenance, retention, and recovery | [Spec 052](../../../completed/03.specs/0052-document-taxonomy-consolidation/spec.md) |
