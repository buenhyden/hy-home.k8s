...
> **rootCA.pem이 로컬 신뢰 저장소에 등록되어 있어야** Headlamp/Kiali HTTPS 접근이 브라우저에서 신뢰된다.
...
export POSTGRES_WRITE_ADDR="172.18.0.15"
export POSTGRES_READ_ADDR="172.18.0.15"
export VALKEY_ADDR="172.18.0.9"
```

`bootstrap-local.sh`는 토큰을 환경 변수나 명령 인자로 받지 않고
`/dev/tty`에서 표시 없이 직접 입력받는다. `VAULT_ADDR`는 HTTPS여야 하고
`VAULT_CA_FILE`은 읽을 수 있어야 하며, 비대화형 또는 인증서 검증을
생략하는 fallback은 없다. 외부 Vault 운영자는 `eso-read-platform`
Kubernetes auth role에 `bound_audiences=vault`를 별도로 설정해야 한다.
클러스터 내부 `vault-backend` HTTP 연결은 로컬 k3d 전용 예외이며
production TLS 구성을 의미하지 않는다.

### 외부 서비스 확인

```bash
# 현재 외부 서비스 EndpointSlice 계약 확인
curl --fail --silent --show-error --cacert "$VAULT_CA_FILE" \
  https://vault.127.0.0.1.nip.io/v1/sys/health | jq '.sealed'
nc -z 172.18.0.15 15432 && echo "postgres-write OK"
nc -z 172.18.0.9 6379   && echo "valkey OK"
nc -z 172.18.0.10 9090  && echo "prometheus OK"  # Kiali용
nc -z 172.18.0.13 3100  && echo "loki OK"        # 로그 수집
nc -z 172.18.0.12 3200  && echo "tempo OK"       # Kiali/트레이싱용
nc -z 172.18.0.11 4317  && echo "alloy OK"       # OTLP gRPC 수신
nc -z 172.18.0.14 3000  && echo "grafana OK"     # Kiali용
```
...
Headlamp(`headlamp.127.0.0.1.nip.io`)와 Kiali(`kiali.127.0.0.1.nip.io`)에 접근하려면
...
ArgoCD가 `platform-headlamp`와 `platform-headlamp-config` 앱을 sync하면:
...
`172.18.0.10/32` (Prometheus) cidr이 누락된 경우 manifest 수정 후 ArgoCD sync.

### Headlamp Token이 Unauthorized
