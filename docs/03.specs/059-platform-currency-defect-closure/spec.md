...
- Recording the decision not to adopt Argo CD `sourceIntegrity` as
  [ADR 0023](../../02.architecture/decisions/0023-argo-cd-source-integrity-non-adoption.md).
...
- Pinning `targetRevision` to commit SHAs. It is the stronger control for the
  recorded identity gap and is named as the preferred alternative in ADR 0023,
...
| VAL-PCDC-006 | ADR 0023 records the case for adoption and a named reversal condition        |
...
[Spec 058](../058-workspace-research-full-corpus-refresh/spec.md).
