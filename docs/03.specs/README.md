...
| [`./041-stage-00-agent-governance-contract/spec.md`](./041-stage-00-agent-governance-contract/spec.md) | Stage 00 agent governance contract specification | Active | PRD-003/ARD-0006와 current accepted ADR-0013 program lineage의 첫 tranche로 provider-neutral machine contract, current 10/30과 target-only 12/48 분리, 네 memory class의 authority/provenance/sensitivity/promotion 경계를 구현한다. ADR-0019는 Spec 046 acceptance 전까지 proposed successor다. Reciprocal [Plan](../04.execution/plans/2026-07-28-stage-00-agent-governance-contract.md)과 [Task](../04.execution/tasks/2026-07-28-stage-00-agent-governance-contract.md)가 실행 증거를 소유하며 provider/runtime/live 결과는 `DEFER`다. | 2026-07-28 |
| [`./042-provider-native-runtime-and-model-evidence/spec.md`](./042-provider-native-runtime-and-model-evidence/spec.md) | Provider native runtime and model evidence specification | Draft | Claude, Codex, Gemini native surface와 model/effort/MCP 설정 및 독립 runtime canary evidence class를 정규화한다. | 2026-07-26 |
| [`./043-agent-harness-loop-lifecycle/spec.md`](./043-agent-harness-loop-lifecycle/spec.md) | Agent harness loop lifecycle specification | Draft | bounded retry, progress detection, checkpoint, compaction, handoff, escalation 계약을 구현한다. | 2026-07-26 |
| [`./044-agent-roster-evaluation-and-admission/spec.md`](./044-agent-roster-evaluation-and-admission/spec.md) | Agent roster evaluation and admission specification | Draft | 기존 10개 역할에 docs-researcher와 quality-engineer를 증거 기반으로 추가하고 12-role/48-adapter roster, eval, model fitness, admission gate를 구축한다. | 2026-07-26 |
| [`./045-agent-governance-ci-qa-cutover/spec.md`](./045-agent-governance-ci-qa-cutover/spec.md) | Agent governance CI and QA cutover specification | Draft | Spec 039 baseline 위에 agent-governance static lane, all-files QA, legacy consumer migration 및 stale surface 제거를 수행한다. | 2026-07-26 |
| [`./046-agent-governance-program-closure/spec.md`](./046-agent-governance-program-closure/spec.md) | Agent governance program closure specification | Draft | Specs 041–045, provider별 독립 canary record, 12/48 parity, all-files QA, independent review를 repository-local closure gate로 통합한다. | 2026-07-26 |
