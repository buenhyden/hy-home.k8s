...
| [`./0076-agent-role-coverage-and-contract-completion/spec.md`](./0076-agent-role-coverage-and-contract-completion/spec.md) | Responsibility-boundary coverage, the roles that close it, and routing completeness | Draft | 라우터가 선언한 7개 책임 경계 중 role이 읽는 것은 6개이고 architecture 경계는 소유자가 없다. `.agents/`, `.claude/`, `.codex/`, `.github/`, `policy/`, 등록되지 않은 `scripts/` 도구, `evals/`도 authoring 소유자가 없다. 이 package가 role 다섯을 admit하고 supervisor 라우팅과 doc-writer 경계를 복구하며, 외부 catalog는 provenance로만 기록한다. | 2026-09-10 |
