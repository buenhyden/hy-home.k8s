...
CI는 `pre-commit`, `repo-quality-static`, `manifest-static`으로 역할이 나뉘어 있고, `main` 대상 push/PR 중심 전략과 맞는다. Shell syntax coverage는 pre-commit과 repo-static/manual verification command에서 처리하며 별도 active CI job이 아니다.
...
2026-05-09 follow-up은 중복 제거의 기준을 `policy SSoT + minimal executable/checklist mirrors`로 고정한다. `git-workflow.md`가 branch policy SSoT이고, `ci.yml`과 `validate-repo-quality-gates.sh`는 enforcement mirror, PR template은 reviewer checklist, `.github/ABOUT.md`는 routing hub다. CI job 간 방어적 중복은 의도된 QA coverage이며 prose duplication으로 제거하지 않는다.
...
- **Goals**:
  ...
  - `.github/ABOUT.md`가 policy body를 중복하지 않고 SSoT 위치만 가리키도록 정리한다.
  - branch prefix parity를 `git-workflow.md` 기준으로 CI와 PR template에서 검증한다.
- **In Scope**:
...
| PLN-006 | 보정 작업 plan/task 문서 추가 및 stage README 인덱스 갱신 | `docs/04.execution/plans/`, `docs/04.execution/tasks/` | REQ-DOC-002 | stage README indexes include new docs |
...
| PLN-008 | `.github/ABOUT.md`를 policy router로 축소 | `.github/ABOUT.md` | REQ-DOC-003 | ABOUT links to policy/enforcement SSoTs and does not mirror full branch prefix policy |
| PLN-009 | PR template branch wording 정밀화 | `.github/PULL_REQUEST_TEMPLATE.md` | REQ-DOC-004 | `main` exception requires matching CI/governance change and `infra` is marked as change type, not prefix |
| PLN-010 | greeting workflow least-privilege 보정 | `.github/workflows/greetings.yml` | REQ-SEC-001 | unnecessary `contents: read` permission removed |
| PLN-011 | branch prefix parity와 ABOUT drift gate 추가 | `scripts/validate-repo-quality-gates.sh` | REQ-VAL-003 | gate parses `git-workflow.md` and compares CI regex/message plus PR template prefix list |
| PLN-012 | follow-up evidence를 local/CI-only/skipped로 분리해 기록 | `docs/04.execution/plans/`, `docs/04.execution/tasks/` | REQ-EVIDENCE-001 | verification summary distinguishes local PASS from optional unavailable tooling |
