...
| [`./2026-07-30-agent-governance-ci-qa-cutover.md`](./2026-07-30-agent-governance-ci-qa-cutover.md) | [Spec 045](../../03.specs/045-agent-governance-ci-qa-cutover/spec.md)의 AGQC-000~005는 dedicated agent-governance CI topology, `CI and legacy-cutover contracts/validators/tests`와 closed schemas/fixtures, consumer-first legacy removal, `.github/README.md` rename, local QA inventory, repository-static checkpoint/provider identity 및 durable memory policy까지 `Done`이다. AGQC-006은 whole-branch review/remediation/full-QA 단계에서 진행 중이며 reciprocal closure/postflight identity는 아직 주장하지 않는다. Hosted CI, branch protection, provider runtime/auth/model discovery, actual evaluation/admission/promotion, remote/live, provider resume/handoff canary는 `DEFER`다. | Active | 2026-07-30 |
