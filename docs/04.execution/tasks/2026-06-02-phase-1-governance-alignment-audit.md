...
| Hook path and event wiring | aligned | `.claude/settings.json`, `.codex/hooks.json`, and `.agents/hooks.json` invoke shared scripts under `docs/00.agent-governance/hooks/*.sh`; all four expected hook scripts exist. | No-op. Historical `.claude/hooks` mentions in progress memory are not rewritten. |
...
| GAP-P1-002 | Historical `docs/00.agent-governance/memory/progress.md` entries still mention older `.claude/hooks/*.sh` paths. | Low: these are dated historical evidence, not active contract surfaces. | No-op. Do not rewrite historical progress ledger entries; current contract points to `docs/00.agent-governance/hooks/*.sh`. |
