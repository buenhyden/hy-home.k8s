...
- Stage 00: 사람용 AI Agent 정책과 책임, Codex/Claude 차이, 실행 절차.
...
- Stage 90: 조사·평가·참고 데이터. 다른 stage의 규칙을 덮어쓰지 않는다.
- Stage 98: Git history 복구를 위한 최소 Migration/Tombstone.
- Stage 99: 문서 profile, 경로, ID, lifecycle, schema와 template.
...
| [90.references](90.references/README.md) | 출처와 시점이 명확한 참고 근거 |
| [98.archive](98.archive/README.md) | 경로·권위 이동과 복구 인덱스 |
...
6. superseded ADR은 decision log에 남긴다. 완료·봉인된 본문을 새 형식에
   맞추려고 다시 쓰지 않는다. 필요한 폐기 경로는 Git-backed Migration과
   최소 Tombstone으로 안내하며 본문 복제본이나 redirect를 만들지 않는다.
