...
5. README 변경 시 경로에 해당하는 여섯 profile form과 README fixture, 품질 게이트가 일치하는지 확인한다.
...
15. Cloud example 문서 지식은 `docs/90.references/cloud-examples/**`의 dated snapshot으로만 유지한다. 퇴역한 `examples/{aws,azure}/docs/**`에는 authored/README route가 없으므로 새 문서를 만들지 않는다.
