...
**2026-03-29 갱신**: EndpointSlice 주소를 `172.19.0.x`(infra_net 실제 서브넷)로 전면 수정한다.
...
- 외부 서비스 인터페이스 계약 (K8s DNS 기준):
  ...
  - `valkey-external.platform.svc.cluster.local:26379` → `172.19.0.12`
- `valkey-external` 구현 모델은 `Service + EndpointSlice(172.19.0.12:26379)`로 고정한다.
...
- `argocd` namespace egress는 Valkey(`172.19.0.12:26379`) + DNS(53/TCP,UDP) + HTTPS(443/TCP)를 허용한다.
