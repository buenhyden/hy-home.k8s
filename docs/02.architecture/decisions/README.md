...
| [`./0013-stage-00-canonical-adapter-model.md`](./0013-stage-00-canonical-adapter-model.md) | Stage 00 canonical core와 native/local adapter-surface ownership 결정 | Accepted | 현재 구현의 baseline이다. ADR-0019는 active successor candidate지만 아직 accepted가 아니므로 Spec 046 closure와 결정 수락 전까지 이 결정을 current contract로 유지한다. |
