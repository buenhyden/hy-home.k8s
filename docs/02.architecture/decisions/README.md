...
| [`./0024-terminal-artifact-identity-and-archive-layout.md`](./0024-terminal-artifact-identity-and-archive-layout.md) | Architecture Description, authored API Spec retirement, closed mandatory/prohibited artifact identity, virtual Stage 98 change identity, stable archive, and exact script-disposition successor | Active | `WDTC-AMEND-001` terminal/current design; WORK-105 must atomically convert ARD-0011, retire the zero-consumer authored API Spec surface while preserving native API evidence, accept this record, and change the registry projection. ADR-0023 remains the accepted transition decision until then. |
