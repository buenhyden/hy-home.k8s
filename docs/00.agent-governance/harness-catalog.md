...
This document is the human-readable catalog for the local agent runtime used in
`hy-home.k8s`. The machine-readable roster, role semantics, permission classes,
memory classes, consumer selection, and evidence classes are owned by
[`contracts/harness-contract.json`](contracts/harness-contract.json). This
catalog derives the supported agents, skills, model allocation, scope imports,
and pattern families used by the shared local runtime bridge (`.claude/`,
`.codex/`, `.agents/`, `.gemini/`).

### Purpose

- Provide a readable projection of the machine-owned local runtime roster.
- Keep gateway files and runtime files in sync.
- Keep native Claude/Codex/Gemini role adapters and local/Antigravity role adapters (`.claude/agents/*.md`, `.codex/agents/*.toml`, `.gemini/agents/*.md`, `.agents/agents/*.md`) statically aligned without claiming provider runtime parity.
...
### Machine Contract and Inventory Boundary

`contracts/harness-contract.json` at version `1.0.0` is the semantic and
inventory machine owner. Its current inventory is exactly `12 roles / 4
surfaces / 48 adapters` across local/Antigravity, Claude, Codex, and the
tracked Gemini-native project adapter surface. `docs-researcher`,
`quality-engineer`, and `.gemini/agents/**` are repository-static current
projections only; they must not be reported as provider discovery, model
resolution, event delivery, authenticated execution, CI, remote, or live
evidence.
...
Select a model tier by task profile, then resolve the provider-specific
incumbent or candidate state through the model-fitness contract.
...
| Architecture constraints           | What prevents unsafe or off-domain action? | `rules/agentic.md`, `subagent-protocol.md`, `model-policy.md`, `document-authoring.md`, `docs/99.templates/`, GitOps-first rules                                                                  | Native command allow/deny policy in `.claude/settings.json`, least-privilege agent `tools:`, shared hooks      | Codex sandbox/approval model plus `.codex/hooks.json` context and validation wiring; Codex hooks are not a permission gate equivalent | Blocks or routes direct live mutation, sensitive-data exposure, unsupported document paths, model drift, and hallucinated runtime claims.                                                                                 |
...
- **Drift garbage collection**: code drift, document drift, and structure drift
  ...
  validators, templates, README indexes, Archive Records, or memory entries.
...
| Local role roster and tracked adapter set | `contracts/harness-contract.json`, `.claude/agents/*.md`, `.codex/agents/*.toml`, `.agents/agents/*.md`, `.gemini/agents/*.md`                                                                                                                                                                                                                                                                                   | The current contract has exactly twelve shared role stems projected through four tracked surfaces into forty-eight adapters; Gemini runtime discovery and model resolution remain separate `DEFER` evidence. |
...
| Local Agents                  | Native Claude/Codex/Gemini plus local/Antigravity adapters: `.claude/agents/*.md`, `.agents/agents/*.md`, `.codex/agents/*.toml`, `.gemini/agents/*.md` | Ready  | The current roster exists with surface-specific metadata, scope imports, guardrails, handoff, and postflight; readiness is repo-static                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Agent Adapters                | Native Claude/Codex/Gemini plus local/Antigravity adapters: `.claude/agents/*.md`, `.codex/agents/*.toml`, `.gemini/agents/*.md`, `.agents/agents/*.md` | Ready  | Matching stems, imports, guardrails, handoff, and postflight are checked by `scripts/validate-repo-quality-gates.sh`; this is repository-static adapter parity, not provider runtime parity                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Memory                        | `contracts/agent-loop-lifecycle.json`, `contracts/agent-checkpoint.schema.json`, `docs/00.agent-governance/memory/`                                  | Ready  | Repo-static validators enforce the four authority-bounded classes, atomic/redacted synthetic checkpoint contract, repository-wins resume, promotion/refresh/expiry/archive-GC/conflict, compaction, handoff, and bounded reviewed feedback destinations. Ignored checkpoints and provider-local memory remain advisory; static PASS is not provider-runtime or actual checkpoint-execution evidence. |
...
| Documentation language and template routing | `document-authoring.md`, `docs/99.templates/README.md`, `docs/README.md`                 | Ready  | None | Keep docs folder responsibilities explicit, route each document family to exactly one canonical template, prefer Korean for human-facing README/overview prose, and prefer English for AI-agent-facing requirements.                                                                                                                                                                                                      |
| Drift garbage collection                    | `rules/agentic.md`, `document-authoring.md`, `scripts/validate-repo-quality-gates.sh`, `docs/98.archive`, `memory/progress.md`                     | Ready  | None | Treat recurring code, document, and structure drift as harness defects; clean temporary/debug artifacts, preserve conflicting old docs as governed Archive Records, and add deterministic validator coverage when possible.                                                                                                                                                                                                            |
| Local Agents                                | `contracts/harness-contract.json` plus native Claude/Codex/Gemini and local/Antigravity role adapters                                                  | Ready  | None | Keep the current twelve roles and forty-eight adapters thin and aligned with the machine contract; keep runtime/provider discovery separate from repository-static adapter parity.                                                                                                                                                                                                                                        |
| Agent Adapters                              | `.claude/agents/*.md`, `.codex/agents/*.toml`, `.gemini/agents/*.md`, local `.agents/agents/*.md`                                                       | Ready  | None | Update all affected tracked role adapters in the same change set; keep adapter parity in the quality gate without claiming provider runtime parity.                                                                                                                                                                                                                                                                       |
...
| Permissions and hooks                       | `.claude/settings.json`, local `.agents/hooks.json`, shared `docs/00.agent-governance/hooks/*.sh`                                                      | Partial | Gemini CLI native settings repo-static only | Keep the destructive Git deny list plus Claude allow/deny and lifecycle hooks; keep local/Antigravity wiring scoped to behavior/validation; do not report `.agents/hooks.json` or `.gemini/settings.json` as Gemini CLI native event, auth, or model-resolution evidence. |
...
| Memory                                      | `contracts/agent-loop-lifecycle.json`, `contracts/agent-checkpoint.schema.json`, `docs/00.agent-governance/memory/`                                  | Ready  | None | Keep the exact four-class authority matrix, durable shared progress in `memory/progress.md`, and domain knowledge in its owner. Repo-static validators enforce atomic/redacted synthetic checkpoint, repository-wins resume, promotion/refresh/expiry/archive-GC/conflict, compaction, handoff, and bounded reviewed feedback destinations; ignored/provider-local context remains advisory and static PASS is not runtime evidence. |
...
| Documentation routing           | `rules/document-authoring.md`, `.agents/skills/docs-stage-routing/skill.md`, `docs/00.agent-governance/hooks/*.sh`, `.codex/hooks.json`         | Ready  | None | Keep generated docs in the canonical stage tree, use templates before authoring, and keep Claude/Codex edit hooks enforcing the template contract.                                        |
| Documentation language boundary | `rules/document-authoring.md`, `docs/README.md`, `scripts/validate-repo-quality-gates.sh`                                                       | Ready  | None | Keep human-facing README and overview prose Korean, governance/runtime control docs English, and AI-agent-specific requirement sections English even inside otherwise human-facing docs.  |
...
tracked provider-native role adapter surfaces. `.agents/agents/*.md` is the
local/Antigravity adapter surface for the same roster. All four must keep the
same name, role, scope imports, guardrails, handoff, and postflight
requirements. Update all affected tracked adapters in the same change set when
a role contract changes. This is repository-static adapter parity, not evidence
of provider discovery or runtime execution.

The canonical inventory is **Twelve shared local role stems / forty-eight tracked role adapters**.
...
| `.claude/skills/requirements-to-design/skill.md`  | workflow          | Connect PRD requirements to AD/ADR artifacts, build a coverage matrix, and surface gaps in docs/01→02 traceability                               | requirements traceability, architecture coverage audit, PRD-to-AD mapping                 | governance, technical-writer                 |
...
| Hooks              | Lifecycle/edit hook contract (`rules/bootstrap.md`, `rules/document-authoring.md`)                | Native settings/hooks plus shared scripts     | Native: absent; local: `.agents/hooks.json`       | `.codex/hooks.json` context/validation wiring                                    | Local wiring Ready; Gemini CLI event delivery `DEFER`                                 |
...
| Template authoring | `rules/document-authoring.md` + `docs/99.templates/README.md` | edit hooks + docs-stage-routing skill         | behavior contract + docs-stage-routing skill      | edit hooks + docs-stage-routing skill                                            | Ready                                                                                 |
