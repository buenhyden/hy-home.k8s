...
| Claude permissions/hooks | `.claude/settings.json`, `.claude/hooks/*.sh` | Partial | Claude runtime has allow/deny command policy plus SessionStart, PreToolUse, and PostToolUse hooks; Stop, SubagentStop, and PreCompact safeguards remain policy/report-driven |
| Codex event hooks | `.codex/hooks.json` | Partial | Codex hook wiring reuses SessionStart, PreToolUse, PostToolUse, and graphify context hooks where supported; it is not a permission gate equivalent and has no Stop/SubagentStop/PreCompact enforcement |
...
| Claude permissions/hooks | `.claude/settings.json`, `.claude/hooks/*.sh` | Partial | No Stop, SubagentStop, or PreCompact hook enforcement for completion and compaction safeguards. | Keep allow/deny command policy plus SessionStart, PreToolUse, and PostToolUse hooks; use reports, handoffs, memory/progress, and postflight checklist as current safeguards unless a future approved plan adds lifecycle hooks. |
| Codex event hooks | `.codex/hooks.json` | Partial | Codex hook wiring is context/validation only and has no Stop, SubagentStop, or PreCompact enforcement. | Keep Codex hook scope limited to context and validation wiring; do not treat it as a Claude permission gate equivalent, and add lifecycle hooks only through a future approved plan. |
