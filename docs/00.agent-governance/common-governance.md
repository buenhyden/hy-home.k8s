...
adapters. Static stem parity across those three surfaces is local adapter
parity, not evidence of three-runtime discovery or enforcement.
...
| **Agent Definition** | `.agents/agents/*.md` (local adapter) | `.claude/agents/*.md` | `.codex/agents/*.toml` |
...
| **Hooks Config** | `.agents/hooks.json` (local behavioral wiring) | `.claude/settings.json` | `.codex/hooks.json` |
...
| Role adapter surfaces | `.agents/agents/*.md` (local/Antigravity), `.claude/agents/*.md` (Claude native), `.codex/agents/*.toml` (Codex native) | Agent roles stay statically aligned, while metadata, model, tool, and permission syntax remains surface-specific. |
...
`.gemini/agents/**` and `.gemini/settings.json`. Neither native surface is
tracked in the current repository. Gemini CLI discovery, event delivery,
policy loading, and model resolution therefore remain `DEFER`; `.agents/**`
must not be used as evidence for any of those runtime behaviors. Adopting the
native Gemini CLI surfaces requires a separately approved PRD/ARD/Spec/Plan/Task
change set, or at minimum an approved Spec/Plan/Task when product and
architecture intent are already established.
...
| **Subagent Protocol** | ⚠️ Local adapter/behavioral | ✅ Native tools frontmatter | ⚠️ TOML adapter/config-based | Local adapter parity is validated; Gemini CLI native discovery and tool enforcement remain `DEFER` |
