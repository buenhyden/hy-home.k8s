...
| `01.requirements` | Product, system, and interface requirements | Before feature implementation | Product Manager / System Architect | Problem statement, business goals | PRD, optional SRS, optional Interface Requirement | `prd.template.md`, `srs.template.md`, `interface.template.md` | Acceptance criteria, scope, interfaces, and success metrics are testable |
| `02.architecture/descriptions` | Architecture descriptions and reference design | After requirement baseline | System Architect | PRD/SRS/Interface Requirement | AD | `ad.template.md` | Boundaries, quality attributes, and data flow are explicit |
| `02.architecture/decisions` | Architecture decision records | When major decisions are made | System Architect | AD, alternatives | ADR | `adr.template.md` | Decision rationale, alternatives, and consequences are traceable |
| `03.specs` | Detailed technical specification | Before implementation | Backend/Frontend/Security Engineer | PRD, SRS/Interface Requirement, AD, ADR | Spec/Agent/Data/Test design docs and native contracts | `spec.template.md`, `agent-design.template.md`, `data-model.template.md`, `tests.template.md`, native OpenAPI/GraphQL/Protobuf forms | Contracts and verification-ready design are complete |
...
| `98.archive` | Immutable full-body archive records | When an old active-stage document conflicts with current implementation or is deprecated-only/superseded-only | Governance Steward | Current replacement docs, exact source Git blob, implementation evidence | ArchiveEnvelope.v1 records and archive index manifest | `archive-record.template.md` | Exact payload, provenance, digest, mirror path, and historical links are preserved; active docs link only to the archive index |
