...
| ID | Benchmark | Expected control | Repository evidence | Maturity | Verdict | Confidence | Gap | Recommendation | Priority | Follow-up owner | Acceptance evidence |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| COMMON-001 | Current AI-agent research; Subagent Protocol | Every role has one shared body skeleton, explicit scope import, bounded bootstrap, guardrails, handoff/escalation, and postflight without forcing provider metadata equivalence. | At the observation SHA every local role body has the same eight required sections; `validate-repo-quality-gates.sh` deterministically checks common runtime phrases, scope parity where applicable, postflight, catalog registration, and stem parity. | 3 deterministic local+CI enforcement | Implemented | Verified repo-static | No missing assessed shared-body shape control; provider-native consumption remains solely owned by Task 7. | Preserve one role-body contract and link provider-specific facts to Task 7. | N/A — no action | N/A — no action | N/A — no action |
...
| COMMON-004 | Current research semantic-validator analysis; Task 7 COM-002 | Static validation detects role responsibility, output, guardrail, and handoff drift for all ten roles without duplicating provider metadata ownership. | The validator checks common phrases for all roles and four wiki-curator phrases. It does not semantically assert most role-specific responsibilities, inputs, outputs, stop/escalation rules, or handoff targets; Task 7 separately records provider-field maps and native-schema gaps. | 2 repository-static | Partial | Verified repo-static | Corrective: generic shape can pass while role meaning drifts, and Task 11 has no deterministic role-contract fixtures distinct from Task 7 provider metadata fixtures. | Add role-body semantic fixtures for the ten responsibilities and keep provider fields/native schemas in Task 7's owner. | P1 near-term integrity | New Stage 04 Task: local-role-semantic-contract-validation | Positive and negative fixtures cover each role's scope, outputs, stop rules, prohibited actions, and handoff targets; a wrong responsibility or missing escalation fails, while no Task 7 provider field is redefined. |
...
| Shared role system | 5 | 5 | 8 | 20 | 40.0% | `1/1/2/1/0` | `1/3/1/0` | `5/0/0` | None |
...
| **Overall** | **31** | **29** | **52** | **116** | **44.8%** | **`1/7/18/3/0`** | **`3/25/1/2`** | **`27/0/2`** | **2 controls** |

The arithmetic is `52 / (4 * 29) = 52 / 116 = 44.8%`. UP-005 and
