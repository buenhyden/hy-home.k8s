...
| Ten shared role bodies and their 20 provider mirrors | All 30 adapters route to postflight; individual roles name bounded validation expectations. | Common handoff exists, but no adapter contains `pre-commit run --all-files`; duplicating the command into 30 bodies would create drift. |
