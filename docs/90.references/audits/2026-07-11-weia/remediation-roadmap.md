...
| `RMD-032` | Closed for intended local-role semantic fixtures. | Current authority remains [Spec 031](../../../03.specs/031-affected-surface-agent-qa/spec.md); completed execution history is indexed through the [Archive Index](../../../98.archive/README.md#document-index), alongside [`validate-agent-role-semantics.py`](../../../../scripts/validate-agent-role-semantics.py). | Fixtures prove the intended local/Claude/Codex tracked role surfaces, not provider-native discovery or behavior. Provider canaries, model fitness, remote execution, and live readiness remain `DEFER`; final unresolved ownership is preserved by [Spec 040](../../../03.specs/040-contract-cutover-and-program-closure/spec.md). |
