...
| RMD-028 | [Governance/provider](governance-harness-loop-providers.md), HAR-001; [CI/QA](ci-qa-automation-pipeline-workflow.md), CICD-009, QA-003, SUP-008 | Machine task state, service-delivery metrics, a Prettier consumer, and changelog retention each lack a named consumer and measured value/cost; absence is not itself a defect. | P3 | All mandatory phases complete; separate consumer evidence per lane | Consolidates 2026-07-05 DORA/optional automation routes | Telemetry-gated: keep deferred; open separate Specs/Tasks only when each lane meets its own trigger and acceptance. |
...
| --- | --- | --- | --- |
| A — safety and integrity | RMD-001, RMD-002, RMD-004, RMD-011, RMD-013, RMD-018, RMD-022 | PRD `workspace-assurance-remediation`, ADR `consolidated-workspace-assurance-target-state`, then Spec `vault-eso-transport-and-secret-exposure-hardening` | An instrumented negative/positive suite proves verified TLS is default, untrusted endpoints fail, no secret appears in argv/env/output/temp capture, break-glass expires, and rollback restores the prior revision; fixed fixtures also reject stale roster facts, omitted validator inputs, conflicting all-files triggers, unchanged over-budget retries, and unclassified retained AI work. |
| B — lifecycle traceability | RMD-005 through RMD-010 | Spec `sdlc-semantic-lineage-and-lifecycle-enforcement`, then Plan/Task `sdlc-traceability-reconciliation` | Fixed-tree fixtures account for every applicable requirement and validation criterion exactly once or by approved exclusion, reconcile family state and reciprocal links, reject invalid transitions/placeholders/dates, preserve historical identifiers, and complete a labeled tabletop without a fake incident. |
| C — provider and harness verification | RMD-015 through RMD-021 | ADR `provider-runtime-intent-and-canary-boundary`, then Task `provider-native-readiness-canaries` and Spec `agent-role-contract-and-evaluation` | Ten-role semantic fixtures pass for every intended surface; each intended provider independently records version/config/discovery/hook or policy/model/MCP PASS/FAIL/SKIP without secrets; retry/handoff recovery rehearses; no role/model promotion occurs without a versioned baseline, threshold, adjudicator, cost/latency, and rollback. |
| D — delivery, supply chain, and platform evidence | RMD-003, RMD-012, RMD-014, RMD-023 through RMD-027 | ARD `platform-supply-chain-assurance-scope`, then Specs for immutable Actions and strict platform assurance | Required tools or named fallback/SKIP are explicit; Action references use reviewed immutable identities; Vault/ESO compatibility, strict GitOps/TLS state, NetworkPolicy behavior, recovery, artifact assurance, and Argo admin boundaries each have approved redacted positive/negative evidence and tested rollback. |
| E — optional optimization | RMD-028 | Separate consumer-specific Spec or Task only after its trigger | Each proposed lane names the consumer, source events/config, privacy/retention, value and latency/cost budget, baseline, rejection threshold, deletion/rollback, and a negative test proving proxy or unused output is not accepted; otherwise disposition remains deferred. |

Phase ordering is dependency order, not a commitment date. P0 work may begin as an
approved minimal safety slice, but no later phase may use static PASS to claim
