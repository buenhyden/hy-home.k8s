...
- Uses `docs/90.references/research/provider-implementation-status.md` as the
...
| Codex skills/MCP/tooling | Codex should expose shared skills and MCP/tooling through trusted config while preserving least-privilege review. | `.codex/skills`, `.codex/workflows`, and `.codex/output-styles` are symlinks to `.agents/**`; no tracked `.codex/config.toml` proves enabled local Codex MCP servers. | Partial | [.agents skills](../../../.agents/skills/workspace-harness-audit/skill.md), [.agents workflows](../../../.agents/workflows/qa-cicd-workflow.md), [common governance](../../00.agent-governance/common-governance.md), [provider research](../research/provider-implementation-status.md) | Shared skills exist, but Codex MCP/tool configuration is not repo-backed in this checkout. | Route MCP additions through a future task with explicit approval for external or mutation-capable tools. |
...
- [Provider Harness Implementation Status Research](../research/provider-implementation-status.md)
...
- **Research benchmark**: [Provider Harness Implementation Status Research](../research/provider-implementation-status.md)
