...
- Review status: fresh WGIA-010 specification/content and quality reviews are
  `Approved` for the four changed row states. WGIA-009 admission remains
  Approved.
- Review disposition: `Approved`; GOV/KNW are implemented, both DOC rows are
  integrated without a duplicate delta, other admitted rows retain their named
  owners, and five rows remain `DEFER` unchanged.
