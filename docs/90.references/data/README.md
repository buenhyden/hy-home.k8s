...
| [Active Corpus Role Audit](./active-corpus-role-audit.json) | repo-backed closed role-audit ledger | ACER-004 exact 24-record Stage 05 corpus and frozen 33-helper ledger, including exact paths, formats, roles, remediation inventory, and zero findings; current safe helpers are validated as a separate post-closure partition without rewriting this ledger or downstream ACER counts | Stage 05 authored-record change, frozen helper drift, current helper/README safety drift, or Spec 037 closure |
...
- `active-corpus-role-audit.json` is ACER-004 repository-static evidence. It
  ...
  remediation. The validator separately reports the current 35-helper corpus
  as 33 frozen plus two post-closure helpers; all current helpers remain
  authoritative safe regular reads, supported non-tracker formats, and exact
  sorted `tests/README.md` members. Neither partition claims live/runtime state.
