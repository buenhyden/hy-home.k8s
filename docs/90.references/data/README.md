...
- `active-corpus-role-audit.json` is ACER-004 repository-static evidence. It
  ...
  remediation. The validator separately reports the current 39-helper corpus
  as 33 frozen plus six exact manifested RIA helpers: the `generator-collision`,
  `minimal-valid`, `overlay-mutation`, `snapshot-mutation`, and `source-freshness`
  JSON fixtures plus the RIA Python regression test, with the canonical
  13/19/6/1 format split. A supported extension or
