...
| **샌드박싱 & 권한** | Filesystem/Network 제한 설정 및 CLI 툴 실행 시 동적 허용/차단. | `read-only`, `workspace-write`, `danger-full-access` 샌드박스 모드 및untusted command 차단. | 개발자가 설계한 도구 스펙(spec) 및 ADK 런타임 보안 정책 준수. |
