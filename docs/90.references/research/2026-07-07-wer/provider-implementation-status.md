...
| OpenAI | Codex product (ChatGPT desktop/web, CLI, IDE) | GPT-5.6 Sol / `gpt-5.6-sol` | Recommended flagship on the Codex product page | Complex, open-ended coding, computer use, research, and cybersecurity | None | **Implementation gap**: product recommendation is current, but local version/account/model resolution was not tested. | [Codex models](https://developers.openai.com/codex/models) |
| OpenAI | OpenAI API catalog | GPT-5.6 Sol / `gpt-5.6-sol`; alias `gpt-5.6` | Listed frontier API model at the cutoff; Responses API access is documented | Complex professional reasoning/coding via API | None | **Unverified locally**: API catalog presence does not prove local API authentication, entitlement, or Codex product access. | [API models](https://developers.openai.com/api/docs/models) |
| OpenAI | Codex product (ChatGPT desktop/web, CLI, IDE) | GPT-5.6 Terra / `gpt-5.6-terra` | Recommended balanced model on the Codex product page | Everyday work and migration starting point for prior GPT-5.5 tasks | None | **Implementation gap**: product route is a candidate, not active local state. | [Codex models](https://developers.openai.com/codex/models) |
| OpenAI | OpenAI API catalog | GPT-5.6 Terra / `gpt-5.6-terra` | Listed frontier API model at the cutoff; Responses API access is documented | Intelligence/cost-balanced API workloads | None | **Unverified locally**: API availability and account access were not canaried. | [API models](https://developers.openai.com/api/docs/models) |
| OpenAI | Codex product (ChatGPT desktop/web, CLI, IDE) | GPT-5.6 Luna / `gpt-5.6-luna` | Recommended fast/affordable model on the Codex product page | Clear, repeatable, high-volume extraction, classification, transformation, and structured summaries | None | **Implementation gap**: product route requires local quality thresholds and a native canary. | [Codex models](https://developers.openai.com/codex/models) |
| OpenAI | OpenAI API catalog | GPT-5.6 Luna / `gpt-5.6-luna` | Listed frontier API model at the cutoff; Responses API access is documented | Cost-sensitive, high-volume API workloads | None | **Unverified locally**: API availability and account access were not canaried. | [API models](https://developers.openai.com/api/docs/models) |
...
| GPT-5.6 Sol | Proposed default for ambiguous/high-value Codex work and escalation from Terra; Terra is fallback when its quality threshold passes. | OpenAI API catalog lists `none`, `low`, `medium`, `high`, `xhigh`, `max`; Codex product says use the lowest sufficient effort. Max is additional single-model reasoning time, not subagent orchestration. | Product and API canaries separately; local auth/version/model resolution; complex task quality; effort sweep; cost/latency; rollback from GPT-5.5. | [Model Policy](../../../00.agent-governance/model-policy.md) → Codex provider/runtime owners → new Stage 03/04 auth-surface-specific migration spec/task. |
| GPT-5.6 Terra | Proposed default for routine implementation/review and fallback from Sol; escalation to Sol. | OpenAI API catalog lists `none`, `low`, `medium`, `high`, `xhigh`, `max`; Codex product positions Terra as the everyday route and warns there is no exact GPT-5.5 effort mapping. | Compare representative supervisor/worker tasks against GPT-5.5 and GPT-5.3-Codex; effort sweep, false-negative review rate, cost/latency, and rollback. | [Model Policy](../../../00.agent-governance/model-policy.md) → Codex provider/runtime owners → coordinated adapter/validator migration task after auth inventory. |
| GPT-5.6 Luna | Fallback/default only for clear, repeatable, high-volume work; escalate to Terra or Sol when thresholds fail. | OpenAI API catalog lists `none`, `low`, `medium`, `high`, `xhigh`, `max`; Codex product routes Luna to structured, repeatable work. | Exact-match/schema validity, sampled precision/recall, retry/escalation rate, tool correctness, throughput, and cost ceiling. | [Model Policy](../../../00.agent-governance/model-policy.md) → Codex provider/runtime owners → WERH-008 routing analysis → bounded-worker canary task. |
