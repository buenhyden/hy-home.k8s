...
| Archive, memory, progress | Documented preservation practice plus documented traceability method | [Michael Nygard, Documenting Architecture Decisions](https://cognitect.com/blog/2011/11/15/documenting-architecture-decisions); [Spec Kit](https://github.github.com/spec-kit/index.html) | 2026-07-12 | ADR practice published 2011; Spec Kit living guidance observed 2026-07-12 | Preserve historical decisions and evidence, distinguish current from superseded authority, keep replacement traceability, and record progress without presenting it as the final contract. | Reject silently rewriting historical bodies, resurrecting obsolete guidance as current, or using progress/memory as route or policy owner; those actions erase provenance or create authority conflicts. | ArchiveEnvelope.v1 traceability and provenance keys, exact payload-to-EOF preservation, fixed archived status, central index-only current navigation, and canonical replacement links; standalone `governance/memory` uses five-key Frontmatter; the canonical `governance/progress-ledger` is Frontmatter-free with `## Work Entries`; `governance/progress-entry` is a Frontmatter-free append fragment with an H3 entry and H4 Metadata, Progress, Memory, Evidence, and Handoff sections. | ADR or Spec Kit practice change; archive metadata, memory retention, progress evidence, or current-owner contract change. | `docs/99.templates/templates/common/archive-record.template.md`; `docs/99.templates/templates/common/memory.template.md`; `docs/99.templates/templates/common/progress.template.md` |
