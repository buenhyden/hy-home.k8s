...
| SEC-012 — Implementation gap (High): immutable Actions dependencies | All checked `uses:` references are version tags; Zizmor's `unpinned-uses` rule is disabled. | A moved or compromised tag can change executed CI code without a repository diff. | Evaluate full commit-SHA pinning plus automated update metadata and rollback. | [Zizmor config](../../../../.github/ABOUT.md) |
