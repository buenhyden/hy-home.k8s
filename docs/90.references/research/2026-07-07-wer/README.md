...
작성일을 보존한다. 기존 일곱 reference와 이 인덱스는 2026-07-10 fact-first
감사에서 현재 저장소 증적과 공식 외부 소스에 맞게 재검토되었고, 문서 타입별
형식 근거 ledger는 2026-07-12에 같은 Current pack의 보강 증적으로 추가되었다.
...
├── document-type-format-and-evidence-contract.md # 문서 타입별 형식 근거와 로컬 적용 결정
├── document-migration-evidence-ledger.md   # 문서 마이그레이션 처분 및 증적 정본
...
| --- | --- | --- | --- |
| [ai-agents-roster-and-gap-analysis.md](ai-agents-roster-and-gap-analysis.md) | `accepted` | 에이전트 로스터 비교 및 `agency-agents`와의 gap analysis | 로스터 및 gap 분석; 실제 local agent configuration은 Stage 00/harness가 소유 |
| [automation-pipeline-workflow-qa.md](automation-pipeline-workflow-qa.md) | `accepted` | 자동화, 파이프라인, 워크플로우 구성 및 DORA metrics 분석 | 파이프라인 분석; 워크플로우 실행 로직이나 CI job 설정을 변경하지 않음 |
| [document-migration-evidence-ledger.md](document-migration-evidence-ledger.md) | `active` | 469개 대상 문서의 처분, 목적지, 로컬 증거, 외부 형식 근거 및 freshness 경계 | 마이그레이션 증적 정본; 원문 사실, accepted ADR, 프로바이더 명세, 런타임 상태의 권한을 대체하지 않음 |
| [document-type-format-and-evidence-contract.md](document-type-format-and-evidence-contract.md) | `active` | 문서 타입별 외부 형식 근거, 채택·거부 결정, freshness trigger | 형식 결정 증적; route, Frontmatter schema, lifecycle 정본은 Stage 99가 소유 |
| [harness-and-loop-engineering.md](harness-and-loop-engineering.md) | `accepted` | 하네스/루프 엔지니어링 개념 및 환경/체계 분석 | 개념 요약서; 런타임 제어나 실행 동작을 직접 변경하지 않음 |
| [kubernetes-infrastructure-security.md](kubernetes-infrastructure-security.md) | `accepted` | 쿠버네티스, 인프라, GitOps, 보안 및 secrets 분석 | 인프라 구조 분석; 실 클러스터 변경 및 Vault/secrets/reconciliation에 관여 불가 |
| [provider-implementation-status.md](provider-implementation-status.md) | `accepted` | Claude, Codex, Gemini 구현 현황 및 공통 체계 구축 | 프로바이더 비교서; 실제 프로바이더의 API/config 설정을 덮어쓰지 않음 |
| [spec-sdlc-ci-qa-formatting.md](spec-sdlc-ci-qa-formatting.md) | `accepted` | 스펙 주도 개발, SDLC, CI/CD, QA, 포맷팅 분석 | 검증 방식 요약; 실제 Actions/pre-commit 구성은 관련 config가 소유 |
| [workspace-governance-baseline.md](workspace-governance-baseline.md) | `accepted` | 워크스페이스 거버넌스 및 운영 baseline 요약 | 설명용 요약서; active governance 정책은 Stage 00이 소유 |
