...
Archive Tombstone은 독립 사업 산출물이 아니라 이전 active-stage 문서의
metadata-only 보존 표면이다. `archived` only이며 original path,
archive reason, replacement 또는 preservation reason을 남겨야 한다.
...
- **Archive**: body를 복제한 두 번째 current surface를 만들지 않고
  metadata-only tombstone에 replacement 또는 보존 이유를 남긴다.
...
Archive Tombstone은 canonical schema에 따라 공통 다섯 key에
`original_path`, `archived_on`, `archive_reason`, `replacement`를 추가로
필수화한다. 따라서 공통 five-key baseline을 모든 profile의 전체
allowed-key set으로 해석하지 않는다.
...
- `status`는 공통 set 포함여부에서 멈추지 않고 PRD/Spec/Plan/Task의
  `done`, ARD/ADR/Operations의 `accepted`, Tombstone의 `archived` 같은
