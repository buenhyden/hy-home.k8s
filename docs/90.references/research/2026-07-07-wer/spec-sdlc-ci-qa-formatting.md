...
| Prettier | Official tool guidance | AST를 parse하고 자체 규칙으로 다시 출력해 일관된 코드 formatting을 제공한다. | tracked Prettier config/hook은 없다. 로컬 formatting은 EditorConfig, hygiene hooks, Markdown lint, `shfmt`, `git diff --check`가 소유한다. |
...
| --- | --- | --- | --- | --- |
| Formatting | `.editorconfig`; `trailing-whitespace`, `end-of-file-fixer`, `mixed-line-ending`; `shfmt`; `git diff --check` | configured editor/file hygiene와 shell formatting, pending diff whitespace 상태 | semantic correctness, Markdown policy 전체, live readiness | Implemented; Prettier는 미구성 |
...
| Syntax / parse | `check-yaml`, `check-json`, `check-toml`; quality gate의 `bash -n`; `validate-k8s-manifests.sh`의 PyYAML parse | 대상 파일이 해당 parser와 shell grammar를 통과함 | Kubernetes schema/admission, application behavior | Implemented; Kubernetes-like YAML은 generic `check-yaml`에서 제외되고 manifest lane이 소유 |
...
- **Formatting**은 텍스트를 정규 형태로 만드는 책임이다. 이 저장소의 실제
  owner는 EditorConfig, file-hygiene hooks, `shfmt`, diff hygiene다. Prettier는
  외부 비교 기준일 뿐 활성 formatter가 아니다.
