...
| Instructions | Claude-facing `CLAUDE.md` imports `AGENTS.md`; Codex gateway is `AGENTS.md`. | Large imported instruction chains can be context-heavy and behavioral only. | Keep gateways thin, canonical policy once, task/scope detail JIT. | Move repeated multi-step method to an owned skill/template, not duplicated gateway prose. |
