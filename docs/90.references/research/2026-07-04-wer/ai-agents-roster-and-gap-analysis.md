...
| `engineering-sre` (SLO/observability)                   | Adopt (candidate) | Observability manifests already exist (`gitops/platform/monitoring/`, `external-services/prometheus-external.yaml`, `grafana-external.yaml`, `kube-state-metrics.yaml`, `alloy-k8s-logs.yaml`, Kiali), so an `observability` worker for monitoring-manifest review and SLO doc upkeep is justified now; requires a Stage 03 agent-design spec first.     |
| `engineering-network-engineer`                          | Adopt (candidate) | A non-trivial ingress/network surface already exists (`traefik/`, multiple `gitops/**/ingress.yaml`, `gitops/platform/network-policies/`, `infrastructure/tests/verify-ingress-tls.sh`) with no dedicated owner; today it falls to `k8s-implementer` + `security-auditor`. A `network` worker is justified; requires a Stage 03 agent-design spec first. |
