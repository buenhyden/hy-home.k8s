...
본 디렉토리는 로컬 k3s/k3d 인프라의 Azure 마이그레이션 프로젝트(hy-home.k8s)에서 비즈니스 목표와 제품 요구사항을 수립하고 문서화하는 공간이다. 시스템의 성공 지표, 워크로드 이전 범위, 2026년 3월 기준의 비기능적 품질 요구를 핵심 산출물로 관리한다.
...
- 제품 요구사항 추적매트릭스(RTM) 기초 데이터

### Out of Scope

- 상세 기술 설계 및 데이터 모델링 (02.ard, 04.specs 참조)
- 개별 기술 의사결정의 아키텍처 배경 (03.adr 참조)
- 마 일스톤 및 담당자 배정 등 운영 계획 (05.plans 참조)

## Structure

```text
01.prd/
├── 2026-03-31-azure-migration.md    # Azure 마이그레이션 핵심 PRD
└── README.md                         # 본 문서
```
...
3. [2026-03-31-azure-migration.md](./2026-03-31-azure-migration.md)는 프로젝트의 Single Source of Truth(SSoT)로 유지한다.
...
1. 요구사항 변경 시 상위 아키텍처(ARD)와 기술 명세(Spec)에 미치는 영향도를 우선 분석한다.
2. 2026년 3월 기준 Azure 기술 표준이 누락되었는지 정기적으로 검토한다.
3. 문서 내의 모든 링크는 상대 경로 규칙을 엄격히 준수한다.
