# 06.tasks (Execution Tasks)

> 이 경로는 Azure 마이그레이션 프로젝트의 개별 작업 진행 상황과 검증 결과를 관리한다.

## Overview

본 디렉토리는 실행 계획(05.plans)을 실제 수행 가능한 가장 작은 작업 단위(Task ID: T-*)로 분해하여 관리한다. 각 작업의 수행 여부, 담당자, 상태 및 검증 증적(Evidence)을 기록하는 "작업 일지" 역할을 한다.
...
- 상위 수준의 이행 로드맵 (05.plans 참조)
- 아키텍처 및 기술 명세 (02.ard, 04.specs 참조)
