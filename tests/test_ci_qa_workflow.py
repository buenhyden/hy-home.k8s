...
class CiQaWorkflowTests(unittest.TestCase):
    ...
    def test_one_qa_job_owns_setup_and_execution(self):
        ...
        self.assertFalse(any('pre-commit run' in run or 'unittest discover' in run for run in runs))
        self.assertEqual(sum('actions/setup-python@' in step.get('uses', '') for job in jobs.values() for step in job['steps']), 1)
        checkout = [s for s in jobs['qa']['steps'] if s.get('uses', '').startswith('actions/checkout@')]
        self.assertEqual(len(checkout), 1)
        self.assertEqual(checkout[0]['with'], {'ref': '${{ github.sha }}', 'persist-credentials': False, 'fetch-depth': 0})
    ...
    def test_summary_fails_closed_for_required_results(self):
        ...
        for event, branch in [('pull_request', 'success'), ('push', 'skipped'), ('workflow_dispatch', 'skipped')]:
            for qa in ('success', 'failure', 'cancelled', 'skipped', ''):
                with self.subTest(event=event, qa=qa):
                    result = subprocess.run(['/bin/bash', '-c', script], env={'EVENT_NAME': event, 'BRANCH_POLICY_RESULT': branch, 'QA_RESULT': qa}, capture_output=True, timeout=5)
                    self.assertEqual(result.returncode, 0 if qa == 'success' else 1)
        for branch in ('failure', 'cancelled', 'skipped', ''):
            result = subprocess.run(['/bin/bash', '-c', script], env={'EVENT_NAME': 'pull_request', 'BRANCH_POLICY_RESULT': branch, 'QA_RESULT': 'success'}, capture_output=True, timeout=5)
