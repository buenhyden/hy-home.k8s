...
| Archive cutover regression | `python3 -m unittest tests/test_archive_cutover.py` | Production worktree snapshot evidence that the cutover is atomic and emits named `ARCHIVE-CUTOVER-INCOMPLETE` diagnostics for any partial state; the GREEN snapshot preserves the immutable 31-record/202-link base proof and derives the ledger-backed 43-record/362-link/43-secret-clean aggregate, registry v8/template authority, manifest closure, and current index-only routing without displaying payload or secret matches |
...
| Active corpus residue closure regression | `python3 -m unittest tests.test_active_corpus_retention.ActiveCorpusResidueClosureContractTests`; `python3 scripts/validate-active-corpus-residue-closure.py --root . --self-test`; `python3 scripts/validate-active-corpus-residue-closure.py --root .` | Twenty-two ACER-005 focused methods within the extended 60-test module plus the isolated 17-case self-test prove the exact 12 migrated-closed rows, 100 current Stage 04 rows as 98 bounded `DEFER` and two Spec 037 `retain` controls, 52 pair keys with exact 48/1/3 cardinality and four explicitly owned partial rows, 13 accepted ADR and 28 done-Spec retain guards while canonical non-accepted ADRs remain outside the guard set, the unchanged ACER-004 24/33/0 dependency, descriptor/index equality for every tracked source/inventory/authority/control input, descriptor-only untracked proposals, final Task worktree-blob prebinding for parent staging, exactly 164 closure-schema `lineageId` fields whose canonical values and paths match the immutable source-ledger `pairKey` joins while the closure output contains neither a `pairKey` key nor Unicode-escape workaround, no `HEAD` or ignored-workspace query, and all eight final finding arrays empty. |
