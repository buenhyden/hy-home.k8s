...
| Agent harness contract fixture | `python3 scripts/validate-agent-harness-contract.py --self-test`; `python3 -m unittest tests/test_validate_agent_harness_contract.py` | Repo-static exact 35-case closed evidence plus focused role/projection, all-current consumer selection, zero legacy consumer, exact validator registration/seven-surface routing, permission, evidence, memory, sensitive-content, and regular-file boundary regressions |
...
| Provider evidence contract fixture | `python3 scripts/validate-agent-provider-evidence.py --root . --self-test`; `python3 -m unittest tests.test_validate_agent_provider_config tests.test_validate_agent_provider_canaries` | Eighteen focused tests plus two closed mutation sets for duplicate keys, cutoff confidence, local/prior observation provenance, current Gemini repository-static surface with absent native discovery, model/fallback/fitness gates, MCP role bounds, secret-like values, redaction, non-transitive verdicts, no-mutation canaries, and explicit-root propagation from a foreign working directory |
