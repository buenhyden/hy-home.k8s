...
| Archive cutover regression | `python3 -m unittest tests/test_archive_cutover.py` | Production worktree snapshot evidence that the cutover is atomic and emits named `ARCHIVE-CUTOVER-INCOMPLETE` diagnostics for any partial state; the GREEN snapshot preserves the immutable 31-record/202-link base proof and derives the ledger-backed 35-record/233-link/35-secret-clean aggregate, registry v8/template authority, manifest closure, and current index-only routing without displaying payload or secret matches |
| Active corpus migration regression | `python3 -m unittest tests/test_active_corpus_migrations.py`; `python3 scripts/validate-active-corpus-migrations.py --root . --self-test`; `python3 scripts/validate-active-corpus-migrations.py --root .` | Eight focused methods plus the exact seventeen-case closed mutation matrix reject partial/skipped/reordered batches, prior-batch evidence drift, active source residue, payload byte drift, wrong first or second rollback parent, index membership drift, direct current archive links, duplicate originals, rogue archive additions, unsafe paths, hostile Git steering, and self-referential batch commits. Production proves two complete pairs/four records, base-plus-additive 35/233, four secret-clean payloads, six repaired consumers in batch 2, and seven unique repaired current Markdown consumers across both batches. |
...
- `tests/test_archive_cutover.py` invokes the local/manual ARWB-003 validator on
  ...
  the current ledger-backed GREEN is 35 records, 233 links, and 35 secret-clean
...
- `tests/test_active_corpus_migrations.py` imports the ACER-003 closed validator
  and proves the first two eligible-prefix batches as exactly two complete
  pairs/four records. Eight focused methods plus the exact seventeen-case
  self-test cover partial/skipped/reordered batches, prior-batch evidence drift,
  source residue, payload drift, the exact rollback-parent chain, index drift,
  current direct archive links, duplicate originals, rogue archive additions,
  unsafe paths, hostile Git steering, and forbidden self-referential batch
  commits. The production result is 35 aggregate archive records, 233
  historical links, four secret-clean new payloads, six repaired consumers in
  batch 2, and seven unique repaired consumers across both batches.
