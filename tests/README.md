...
| Agent harness contract repository check | `python3 scripts/validate-agent-harness-contract.py --root .` | Repo-static current `10/3/30`, target-only `12/4/48`, four evidence classes, four memory classes, fourteen `harness-contract/1.0.0/current` consumers, and readable legacy compatibility; no provider-runtime, CI, remote, or live inference |
| Agent roster admission fixture | `python3 scripts/validate-agent-roster-admission.py --self-test`; `python3 -m unittest tests/test_validate_agent_roster_admission.py` | Exact 51-case closed mutation matrix plus eleven focused tests for current/target separation, two candidates, seven admission conditions, four evaluation/memory classes, six deferred evidence lanes, eight surface plans, rollback, closed schema, path/symlink, and sensitive-memory boundaries |
| Agent roster admission repository check | `python3 scripts/validate-agent-roster-admission.py --root .` | Repo-static `state=contract-only verdict=DEFER`, current `10/3/30`, target-only `12/4/48`; no candidate, provider runtime, hosted CI, remote, or live promotion |
...
| Agent model-fitness fixture | `python3 scripts/validate-agent-model-fitness.py --self-test`; `python3 -m unittest tests/test_validate_agent_model_fitness.py` | Exact 16-case closed mutation matrix plus seventeen focused tests for authoritative cutoff, harness observation separation, local/Claude/Codex/Gemini API/CLI boundaries, 48 provider tuples, same-suite baseline, quality/safety priority, fallback, no promotion preclaim, and fail-closed root/input symlink, intermediate-path, non-regular, containment, and non-disclosing CLI boundaries |
...
| Provider evidence contract fixture | `python3 scripts/validate-agent-provider-evidence.py --self-test`; `python3 -m unittest tests.test_validate_agent_provider_config tests.test_validate_agent_provider_canaries` | Seventeen focused tests plus two closed eight-case mutation sets for duplicate keys, cutoff confidence, local/prior observation provenance, target-only Gemini, model/fallback/fitness gates, MCP role bounds, secret-like values, redaction, non-transitive verdicts, and no-mutation canaries |
...
- `tests/fixtures/agent-roster-admission.json`은 51개 closed mutation의
  `AREA-ADM-*` 기대 rule과 current `10/3/30`, target `12/4/48`, 두 candidate,
  일곱 admission condition, 네 evaluation/memory class, 여섯 deferred
  evidence lane, 여덟 surface plan을 고정한다.
...
- `tests/fixtures/agent-provider-runtime-evidence.json`은 provider config와
  ...
  cutoff, separate prior/current observations, target-only Gemini, candidate
