...
| Reference information architecture regression | `python3 -m unittest tests/test_reference_information_architecture.py`; `python3 scripts/validate-reference-information-architecture.py --self-test`; `python3 scripts/validate-reference-information-architecture.py --root . [--require-settled-baselines]` | RIA-002 schema-v2 Historical/Current separation, exact Current map and registry projection, fixed Git object/index reader, immutable snapshot, bounded overlay, root/open/settled FSM, staged C3, and explicit-ref settlement-lineage evidence; repository-static PASS does not claim CI, provider, remote, or live state. |
| Active corpus residue closure regression | `python3 -m unittest tests.test_active_corpus_retention.ActiveCorpusResidueClosureContractTests`; `python3 scripts/validate-active-corpus-residue-closure.py --root . --self-test`; `python3 scripts/validate-active-corpus-residue-closure.py --root .` | ACER-006 extends the focused contract and isolated 19-case self-test for 12 migrated rows, 100 current `DEFER`/0 `retain`, exact 48/1/3 cardinality with four partial owned `DEFER`, terminal Spec 037 source-fact preservation, 13 accepted ADR and 29 done-Spec guards, exact Spec/Plan/Task blob binding, and eight empty findings. Parent staging remains required for production index equality. |
