...
| Agent harness contract repository check | `python3 scripts/validate-agent-harness-contract.py --root .` | Repo-static current `10/3/30`, target-only `12/4/48`, four evidence classes, four memory classes, eleven `harness-contract/1.0.0/current` consumers, and readable legacy compatibility; no provider-runtime, CI, remote, or live inference |
...
| Affected-surface fixture | `python3 scripts/validate-affected-surfaces.py --self-test` | Repo-static exact-route, argv, output, NUL-transport, and focused `agent-harness-contract` selection evidence |
...
agent-role-semantic, and roster-currentness validators in strict blocking mode,
...
- `tests/fixtures/agent-harness-contract.json`은 정확한 21개 mutation과 기대
  ...
  exact consumer/version, duplicate-key·schema·sensitive payload·symlink 및
