...
| Agent harness contract fixture | `python3 scripts/validate-agent-harness-contract.py --self-test`; `python3 -m unittest tests/test_validate_agent_harness_contract.py` | Repo-static exact 21-case closed mutation evidence plus focused role/projection, consumer, permission, evidence, memory, sensitive-content, and regular-file boundary regressions |
| Agent harness contract repository check | `python3 scripts/validate-agent-harness-contract.py --root .` | Repo-static current `10/3/30`, target `12/4/48`, four evidence classes, four memory classes, and eleven pinned consumers; no provider-runtime, CI, remote, or live inference |
