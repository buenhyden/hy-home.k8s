...
| Archive cutover regression | `python3 -m unittest tests/test_archive_cutover.py` | Production worktree snapshot evidence that the cutover is atomic and emits named `ARCHIVE-CUTOVER-INCOMPLETE` diagnostics for any partial state; the GREEN snapshot preserves the immutable 31-record/202-link base proof and derives the ledger-backed 37-record/275-link/37-secret-clean aggregate, registry v8/template authority, manifest closure, and current index-only routing without displaying payload or secret matches |
| Active corpus migration regression | `python3 -m unittest tests/test_active_corpus_migrations.py`; `python3 scripts/validate-active-corpus-migrations.py --root . --self-test`; `python3 scripts/validate-active-corpus-migrations.py --root .` | Ten focused methods plus the exact twenty-case closed mutation matrix reject partial/skipped/reordered batches, explicit first/second prior-batch evidence drift, active source residue, payload byte drift, wrong first, second, or third rollback parent, index membership drift, direct current archive links, duplicate originals, rogue archive additions, unsafe paths, hostile Git steering, and self-referential batch commits. Production proves three complete pairs/six records, base-plus-additive 37/275, six secret-clean payloads, five repaired consumers in batch 3, and eight unique repaired current Markdown consumers across all batches. |
...
- `tests/test_archive_cutover.py` invokes the local/manual ARWB-003 validator on
  ...
  the current ledger-backed GREEN is 37 records, 275 links, and 37 secret-clean
