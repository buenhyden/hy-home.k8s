...
| Archive cutover regression | `python3 -m unittest tests/test_archive_cutover.py` | Production worktree snapshot evidence that the cutover is atomic and emits named `ARCHIVE-CUTOVER-INCOMPLETE` diagnostics for any partial state; the GREEN snapshot preserves the immutable 31-record/202-link base proof and derives the ledger-backed 41-record/329-link/41-secret-clean aggregate, registry v8/template authority, manifest closure, and current index-only routing without displaying payload or secret matches |
| Active corpus migration regression | `python3 -m unittest tests/test_active_corpus_migrations.py`; `python3 scripts/validate-active-corpus-migrations.py --root . --self-test`; `python3 scripts/validate-active-corpus-migrations.py --root .` | Thirteen focused methods plus the exact twenty-eight-case closed mutation matrix reject partial/skipped/reordered batches, explicit first/second/third/fourth prior-prefix drift, active source residue, payload byte drift, wrong first through fifth rollback parent, index membership drift, direct current archive links, duplicate originals, rogue archive additions, unsafe paths, hostile Git steering, and self-referential batch commits. Production proves five complete pairs/ten records, base-plus-additive 41/329, ten secret-clean payloads, five repaired consumers in batch 5, and thirteen unique repaired current-or-migrated-owner Markdown consumers across all batches. |
...
- `tests/test_archive_cutover.py` invokes the local/manual ARWB-003 validator on
  ...
  the current ledger-backed GREEN is 41 records, 329 links, and 41 secret-clean
...
- `tests/test_active_corpus_migrations.py` imports the ACER-003 closed validator
  and proves the first five eligible-prefix batches as exactly five complete
  pairs/ten records. Thirteen focused methods plus the exact twenty-eight-case self-test
  cover partial/skipped/reordered batches, explicit first/second/third/fourth prior-prefix
  drift, source residue, payload drift, the exact five-parent rollback chain,
  ...
  batch commits. The production result is 41 aggregate archive records, 329
  historical links, ten secret-clean new payloads, five repaired consumers in
  batch 5, and thirteen unique repaired consumers across all batches.
