...
| Agent roster admission fixture | `python3 scripts/validate-agent-roster-admission.py --self-test`; `python3 -m unittest tests/test_validate_agent_roster_admission.py` | Exact 59-case closed mutation matrix plus thirteen focused tests for repository-static-only authorization, achieved/current separation, two admitted candidates, seven admission conditions, four evaluation/memory classes, nine deferred evidence classes, eight surface plans, rollback, closed schema, path/symlink, and sensitive-memory boundaries |
| Agent roster admission repository check | `python3 scripts/validate-agent-roster-admission.py --root .` | Repo-static `state=repository-static-admitted verdict=PASS`, current `12/4/48`, achieved target mirror `12/4/48`; no provider runtime, hosted CI, remote, or live promotion |
...
| Agent role semantics fixture | `python3 scripts/validate-agent-role-semantics.py --self-test` | Repo-static 768-case category mutation evidence plus closed Gemini-native model/tool metadata mutations |
...
- `tests/fixtures/agent-roster-admission.json`은 59개 closed mutation의
  ...
  `12/4/48`, 두 repo-static admitted role record, 일곱 admission condition,
...
- `tests/fixtures/agent-role-semantics.json`은 정확히 12개 role, local/Claude/
