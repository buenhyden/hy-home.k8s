...
| CI Python contract regression | `python3 -m unittest tests/test_validate_ci_python_contract.py`; `python3 scripts/validate-ci-python-contract.py --self-test`; `python3 scripts/validate-ci-python-contract.py --root .` | 29개 focused test와 유지된 14-case self-test가 임시 최소 저장소 및 실제 root에서 exact three-line Python requirements/inventory mirror, 네 validation job의 Python 3.12 및 단일 shared install, validation set 밖 job의 setup-python/pip-install 부재, `pre-commit/action` 부재, exact all-files/show-diff 실행, `pre-commit`과 `repo-quality-static`의 exact verified Gitleaks `8.30.0` 설치, 두 job의 full-history checkout을 검증한다. Symlink root와 lexical `..` root, requirements/workflow/inventory 각각의 symlink parent 및 final symlink/directory/FIFO node와 descriptor-bound parent/final identity swap을 독립적으로 차단한다. 열한 stable rule ID의 exact duplicate-free inventory, 그 inventory에서 파생된 self-test rule count, value-free root/owner diagnostics를 fail-closed로 검사하며 validator 자체는 네트워크를 사용하지 않는다. |
