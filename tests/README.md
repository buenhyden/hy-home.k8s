...
| Agent legacy cutover fixture | `python3 scripts/validate-agent-legacy-cutover.py --root . --self-test`; `python3 -m unittest tests/test_validate_agent_legacy_cutover.py` | Closed zero-consumer, retired-path, canonical replacement, historical-evidence, path-boundary, duplicate-key, and GitHub hub cutover mutation evidence |
| Agent legacy cutover repository check | `python3 scripts/validate-agent-legacy-cutover.py --root .` | Repository-static proof that retired role-semantics compatibility surfaces and the retired GitHub hub path are absent, current consumers select harness owners, and `.github/README.md` is canonical; no hosted/provider/runtime/remote/live claim |
