...
| Agent-governance CI fixture | `python3 scripts/validate-agent-governance-ci.py --root . --self-test`; `python3 -m unittest tests/test_validate_agent_governance_ci.py` | Exact `truth_cases=6 mutation_cases=43` inventory and twenty-five focused artifact, schema, semantic, path/symlink/non-regular, duplicate-key, topology, inherited-secret, skipped-step, custom/default-shell, exact-run-sequence, exact-summary-shape/digest, job/step fail-open, security, routing, explicit-root aggregate invocation, local-QA order/inventory, and evidence-boundary regressions |
...
- `tests/fixtures/agent-governance-ci.json`은 selected/result 조합의 exact
  six-row truth table과 43개 closed mutation을 고정한다.
  `tests/test_validate_agent_governance_ci.py`의 24 focused tests는 adjacent
  schema, unknown key/version, result/evidence vocabulary, selector/job/summary,
  twelve route classes, sixteen delegated checks, aggregate/pre-commit/CI order,
