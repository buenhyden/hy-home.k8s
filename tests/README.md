...
| Agent legacy cutover fixture | `python3 scripts/validate-agent-legacy-cutover.py --root . --self-test`; `python3 -m unittest tests/test_validate_agent_legacy_cutover.py` | Closed `positive_cases=3 mutation_cases=23` zero-consumer, retired-path, canonical replacement, historical-evidence, duplicate-key, Git candidate-source, top-level-root, unsafe-path, NUL transport, ignored-unreadable sentinel, symlink/non-regular, and GitHub hub cutover evidence plus twenty-three focused tests |
| Agent legacy cutover repository check | `python3 scripts/validate-agent-legacy-cutover.py --root .` | Repository-static deterministic cached plus non-ignored-untracked candidate result `scanned_files=809 evidence_references=43 active_consumers=0`; ignored paths are never opened, retired role-semantics compatibility surfaces and the retired GitHub hub path are absent, current consumers select harness owners, and `.github/README.md` is canonical; no hosted/provider/runtime/remote/live claim |
