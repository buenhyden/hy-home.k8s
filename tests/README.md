...
| Archive cutover regression | `python3 -m unittest tests/test_archive_cutover.py` | Production worktree snapshot evidence that the cutover is atomic and emits named `ARCHIVE-CUTOVER-INCOMPLETE` diagnostics for any partial state; the GREEN snapshot preserves the immutable 31-record/202-link base proof and derives the ledger-backed 33-record/218-link/33-secret-clean aggregate, registry v8/template authority, manifest closure, and current index-only routing without displaying payload or secret matches |
| Active corpus migration regression | `python3 -m unittest tests/test_active_corpus_migrations.py`; `python3 scripts/validate-active-corpus-migrations.py --root . --self-test`; `python3 scripts/validate-active-corpus-migrations.py --root .` | Six focused methods plus the exact thirteen-case closed mutation matrix reject partial/wrong/reordered pairs, active source residue, payload byte drift, wrong rollback, index membership drift, direct current archive links, duplicate originals, unsafe paths, hostile Git steering, and self-referential batch commits. Production proves one complete pair/two records, base-plus-additive 33/218, two secret-clean payloads, and six repaired current Markdown consumers. |
...
- `tests/test_archive_cutover.py` invokes the local/manual ARWB-003 validator on
  ...
  the current ledger-backed GREEN is 33 records, 218 links, and 33 secret-clean
...
- `tests/test_active_corpus_migrations.py` imports the ACER-003 closed validator
  and proves the first eligible-prefix batch as exactly one complete pair/two
  records. Six focused methods plus the exact thirteen-case self-test cover
  partial/wrong/reordered batches, source residue, payload drift, rollback and
  index drift, current direct archive links, duplicate originals, unsafe paths,
  hostile Git steering, and forbidden self-referential batch commits. The
  production result is 33 aggregate archive records, 218 historical links, two
  secret-clean new payloads, and six repaired current Markdown consumers.
