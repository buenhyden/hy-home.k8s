...
class BoundedValidationCommandTest(unittest.TestCase):
    ...
    def test_observation_carries_the_escape_that_failed_the_gate(self):
        ...
        escaped = replace(
            ...
            escaped_descendants=((4321, 4321, "gitleaks", "S"),),
        )
        rendered = RUNNER.observation(escaped)
        self.assertIn("status=descendant_cleanup", rendered)
        self.assertIn("escaped=4321:4321:gitleaks:S", rendered)
    ...
    def test_process_state_is_bounded_and_rejects_malformed_fields(self):
        ...
        for raw, expected in (
            ...
            with (
                ...
                patch.object(Path, "open", return_value=BytesIO(raw)),
            ):
                self.assertEqual(RUNNER._process_state(12345), expected)
    ...
    def test_escaping_descendant_identity_is_reported(self):
        ...
        with tempfile.TemporaryDirectory(prefix="runner-escape-identity-") as tmp:
            ...
            try:
                ...
                _pid, pgid, command, state = reported[escaped["pid"]]
                self.assertEqual(pgid, escaped["pgid"])
                # Kernel name/state are diagnostic only; arguments stay unread.
                self.assertTrue(command)
                self.assertNotIn(" ", command)
                self.assertIn(
                    state,
                    {"R", "S", "D", "T", "t", "X", "Z", "P", "I", "K", "W", "unknown"},
                )
            finally:
                if escaped:
                    try:
