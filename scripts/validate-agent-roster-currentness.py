#!/usr/bin/env python3
"""Compatibility CLI for registry-derived projection currentness validation."""
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate current role projections through the agent registry."
    )
    ...
    try:
        ...
        print(
            f"[PASS] agent roster currentness{suffix}: {compat.format_counts(counts)}"
        )
        return 0
