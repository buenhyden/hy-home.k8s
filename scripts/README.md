...
├── validate-agent-role-semantics.py # Shared semantic contract validation for thirty tracked role adapters
...
| -------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `validate-repo-quality-gates.sh` | Keep | Tier A: CI `repo-quality-static`와 post-edit repository quality hook이 직접 실행한다. | root README, `scripts/README.md`, PR template, `.github/ABOUT.md`, `.claude/settings.json`, `docs/00.agent-governance/hooks/post-validate.sh`, `.codex/hooks.json`, docs quality guidance | 문서 구조, registry/Markdown/link 계약, stage index, active 01-05 currentness, operations/security/GitOps/infrastructure matrices, workflow·script·agent inventory를 검증한다. ARWB-003 full-history/secret-classifier cutover는 이 aggregate에서 실행하지 않으며 explicit local/manual proof와 Spec 039 후속 CI 통합으로 분리한다. |
...
| `archive_cutover.py` | Keep | ARWB-003 local/manual production cutover proof; it is intentionally absent from `validate-repo-quality-gates.sh` and the unchanged CI workflow. | Fails closed unless all 31 mirrored ArchiveEnvelope.v1 records, 202 historical links, exact source commits and blobs, payload digests, replacements, index manifest rows, registry v8 profiles, canonical template, current-authority exclusions, and retired-authority cleanup pass atomically. It classifies only exact recovered Git-blob payloads with `gitleaks`, suppresses classifier output, uses recovery-grade isolated Git execution, rejects tables outside the canonical manifest, and reports stable path-only diagnostics. Spec 039 owns later CI integration with pinned `gitleaks` and full-history checkout. | Run `python3 scripts/archive_cutover.py --root .` explicitly in a full-history local worktree with `gitleaks` installed; exit `0` reports only `records=31 historical_links=202 secret_clean=31`, while any partial state emits `ARCHIVE-CUTOVER-INCOMPLETE` plus fixed diagnostics. It does not traverse ignored `_workspace` children. |
| `run-validation-lane.py` | Keep | Tier B Spec 031 local consumer used by shared post-edit and lifecycle hooks | Imports validation-surface schema v2, consumes NUL path records, selects `affected` or `all-files` validator IDs, and invokes only approved argv arrays with `shell=False`. Every validator runs with a closed environment, fixed absolute trusted PATH, and absolute resolved interpreter; ambient shell/Python/Node startup variables are absent. Repository quality requires return code zero and exactly one `[PASS] repository quality gates passed` stdout line. Existing affected Markdown propagates through repeated `--include-path`; remote/live is `DEFER`, while no paths and missing optional tools retain their bounded `SKIP`/fallback semantics. Child stdout/stderr never enters the result; byte counts and SHA-256 digests provide bounded non-secret metadata. | `--root`, `--lane affected\|all-files`, `--paths-file`, required `--delimiter nul`; stable `PASS`/`SKIP`/`FAIL`/`DEFER` lines include absolute command/tool, scope, limitation, and evidence class. Exit `1` means a contract, marker, selection, required-tool, or command failure. |
...
첫 번째 명령은 정확히 10 roles × 3 adapter surfaces × 8 categories ×
...
480개 모두 adapter source를 실제 YAML/TOML/Markdown parser에 다시 통과하며,
...
`validate_registry()`와 `classify_paths()`를 사용하는 mutation 117개,
complete literal v7 typed projection, 64-profile/30-template inventory,
...
| `python3 scripts/archive_cutover.py --root .` | `--root`는 exact repository worktree root다. | exact 31 ArchiveEnvelope.v1 records, 202 historical links, source provenance/digest, gitleaks secret classification, replacement existence, registry v8/template authority, manifest membership, current index-only navigation, retired-authority cleanup을 원자적으로 검증한다. | exit `0`은 aggregate `records=31 historical_links=202 secret_clean=31`만 출력한다. 실패는 `ARCHIVE-CUTOVER-INCOMPLETE`와 fixed path-only diagnostics를 출력하며 payload/secret match를 노출하지 않는다. |
