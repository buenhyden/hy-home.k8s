...
├── document_contracts.py             # Closed registry v6 loader, typed program-lineage projection, inventory, and classifier library
...
├── validate-document-contract-registry.py # Registry v6 schema, typed program lineage, inventory, routes, Stage 00 owners, and Current reference packs
...
| `validate-document-contract-registry.py` | Keep | Tier A document contract registry gate | Repository-static closed-v6 `jsonschema` preflight, mode-aware Git inventory, exactly-one route classification, normalized tracked-regular Stage 00 current-authority declarations, typed Current research/audit packs, and `Registry.program_lineage` validation. Program checks reject legacy-v5 production input, duplicate/overlapping members, non-contiguous order, non-accepted or chronologically invalid decisions, state drift, invalid evidence modes, and numeric owners that are not exactly one tracked regular profile-correct file. The self-test alone contains the bounded v5-to-v6 migration converter. The profile projection fixes all 64 profile semantics and removes every authored/README route for retired `examples/{aws,azure}/docs/**`; a Git-index-only guard rejects tracked reintroduction. 기본 inventory는 tracked-only이고 named untracked candidate만 `lstat()`으로 검사하며 ignored path와 symlink는 열거나 순회하지 않는다. `.gemini/agents/**`와 `.gemini/settings.json`은 Gemini CLI native 표면을 위해 예약되며, validator는 내용을 읽지 않고 `git ls-files -z` index metadata만으로 tracked 선언을 거부한다. | `--root`, `--mode compatibility\|strict`, `--profile`, repeatable `--include-path`, `--self-test`; `PASS`/exit `0`은 선택 scope 통과, `FAIL`/non-zero는 rule-ID 또는 입력 경계 위반을 뜻한다. |
