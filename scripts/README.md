...
| `validate-document-contract-registry.py` | Keep | Tier A document contract registry gate | Repository-static v3 `jsonschema` preflight, mode-aware Git inventory, exactly-one route classification, normalized tracked-regular Stage 00 current-authority declarations, and the required typed Current research/audit pack declaration. The profile-only `DocumentProfileContract.v1` projection remains unchanged. 기본 inventory는 tracked-only이고 named untracked candidate만 `lstat()`으로 검사하며 ignored path와 symlink는 열거나 순회하지 않는다. | `--root`, `--mode compatibility\|strict`, `--profile`, repeatable `--include-path`, `--self-test`; `PASS`/exit `0`은 선택 scope 통과, `FAIL`/non-zero는 rule-ID 또는 입력 경계 위반을 뜻한다. |
