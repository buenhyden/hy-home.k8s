...
비교한다. `stale-count`는 `8 local agents`, `Eight local provider adapters`,
`eight shared roles`, `8 role stems`를 각각 독립적으로 검증한다. 두 번째
명령은 세 provider adapter stem set, 30-file inventory, 대소문자를 구분하지
않는 numeric/spelled stale eight-role prose, canonical harness Markdown label과
정확한 상대 link target 쌍을 확인한다. 두 명령 모두 repo-static 검사이며
provider runtime readiness를 입증하지 않는다.
