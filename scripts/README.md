...
`validate-repo-quality-gates.sh`는 일반 문서 규칙을 다시 구현하지 않는다.
Python/PyYAML/JSON Schema 전제 조건을 확인한 뒤 registry, Markdown profile,
cross-document validator를 compatibility mode로 호출하고, operations index,
archive/snapshot, GitOps, infrastructure, agent runtime, CI/QA, security,
version/supply-chain 같은 workspace domain 검사를 계속 소유한다. Wrapper는
Spec 030 이행 중 fixture 전체의 semantic SHA와 mutation proof를 유지하지만,
runtime debt 판정은 canonical Markdown validator에 위임한다.
Spec 029 closure 이후 cross-document self-test는 완료된 Spec/Plan/Task 세
경로만 current-owner 후보에서 빠졌음을 재현하여 unique key가 `66 → 63`으로
전이했는지 검증한다.
