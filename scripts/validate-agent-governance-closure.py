#!/usr/bin/env python3
"""Compatibility CLI for registry-derived governance closure."""
...
def validate_repository(root: Path) -> dict[str, int]:
    """Preserve the closure API while delegating its sole current authority."""
    return compat.validate(root, MODE)


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate governance closure through the terminal agent registry."
    )
    ...
    try:
        ...
        print(
            f"[PASS] agent-governance closure{suffix}: {compat.format_counts(counts)}"
        )
        return 0
