...
- **No authenticated read-back of this repository's protection has been
  recorded.** The remote state of `branches/main/protection` and of any
  repository ruleset is `unverified`. Treat every row under Target Ruleset as
  intent awaiting verification, never as observed configuration.
- A read-back belongs in the active Task with its date, the exact fields read,
  and the account that read them. Copy no protection claim into this file
  without that record.
...
Because no read-back for this repository has been recorded, there is no
captured before-state to roll back to. Capture one before changing any field,
and record it in the active Task rather than here. Do not import a before-state
from another repository. Local Git recovery restores tracked definitions only;
it never restores remote settings.
