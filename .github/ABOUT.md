...
- `repo-quality-static` also enforces `docs/98.archive` Tombstone structure and active-doc stale runtime contract rejection.
...
| --- | --- | --- | --- | --- |
| `ci.yml` | Required QA gate for branch policy, repo-quality, manifest, secret checks. | Runs on `push`, `pull_request`, and `workflow_dispatch` for `main`-centered integration. | `ci-summary` aggregates `branch-policy`, `changes`, `pre-commit`, `repo-quality-static`, and `manifest-static`; repo-quality includes `.agents/**` shared asset changes, archive Tombstone/currentness checks, and template enforcement. | No deploy CD, direct Kubernetes mutation, external Vault mutation, container publish, or commit push. |
