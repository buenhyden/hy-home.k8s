...
1. **Phase 1: Pre-Development (Planning & Design)**
   ...
   - *Outputs*: PRD (`docs/prd/`), ADRs (`docs/adr/`), and final implementation Spec (`docs/specs/`) and Plan (`docs/plans/`).
...
2. **Phase 2: During-Development (Implementation & Testing)**
   ...
   - *Inputs*: The approved Spec from `docs/specs/` and Plan from `docs/plans/`.
...
3. **Phase 3: Post-Development (Code Review & Operations)**
   ...
   - *Execution*: Reviewer Agent verifies spec compliance and security. DevOps Agent creates or updates deployment runbooks in `docs/runbooks/`.
