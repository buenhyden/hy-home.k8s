...
glob: ["docs/plans/**", "docs/specs/**", "src/**", "tests/**"]
...
- **[REQ-IMP-01] Plan-First Execution**
  - **Rule**: Code modification MUST NOT begin until a `docs/plans/YYYY-MM-DD-<feature-name>.md` (or `implementation_plan.md` for refactors) exists and is up-to-date.
...
- **[REQ-IMP-04] Spec-Anchored Drift Prevention**
  - **Rule**: If the implementation diverges from the original plan, the `docs/specs/`, `docs/plans/` and `docs/` MUST be updated to reflect the true state of the code before proposing a merge.

### Lifecycle Phases

| Phase | Requirement ID | Mandatory Action |
| --- | --- | --- |
| **1. Planning** | [REQ-IMP-05] | Create/Update `docs/plans/YYYY-MM-DD-<feature-name>.md`. Identify touched files. |
...
| **4. Drift Check** | [REQ-IMP-08] | Compare final code to `docs/specs/` and `docs/plans/`. Update specs if drifted. |
...
- **[BAN-IMP-01] "Blind" Coding**
  - DO NOT generate code based solely on the user prompt. ALWAYS cross-reference with the established  `docs/`.
...
- **[PROC-IMP-03] The Drift Check Loop**
  - IF implementation details diverged from the initial `docs/plans/YYYY-MM-DD-<feature-name>.md` or `docs/specs/YYYY-MM-DD-<feature-name>.md` THEN MUST update the text of those spec files to match reality BEFORE declaring completion.
...
- **[VAL-IMP-03] Drift Sync**
  - [ ] Final `git diff` aligns exactly with the logical architecture described in the updated `docs/specs/` and `docs/plans/`.
