...
- **No Secrets**: NEVER commit `.env`, keys, or credentials. Use `.gitignore`. (See [018-core-third-party-integration-specific.md](./018-core-third-party-integration-specific.md) for integration specifics).
...
- **Breaking Changes**: Use `!` before colon or `BREAKING CHANGE:` footer.
- **Case Sensitivity**: Types and scopes are lowercase. `BREAKING CHANGE` is uppercase.

## 7. Atomic Commit Construction (Mandatory)

### Phase 1: Arrangement (Logical Grouping)

- **Preview**: Present a verbose "Arranged Commits Preview" before ANY execution.
- **Hunk-Based**: Use `git add -p` for files with mixed concerns.
- **Authorization**: Wait for explicit "start" or "commit" from the user after preview.

### Phase 2: Execution

- **Step-by-Step**: Execute aligned commits sequentially.
- **Pull**: Always `git pull` (or rebase) before identifying divergence.

## 8. Submodule Management

- **No Detached HEAD**: Always checkout a branch (`main`/`master`) before working in a submodule.
- **Update Workflow**:
  1. `cd submodule` -> `git checkout main` -> `git pull`.
  2. Make changes -> Commit.
  3. `cd ..` -> `git add submodule` -> `git commit -m "chore(submodule): update pointer"`.

## 9. Text Normalization

- **Encoding**: Ensure UTF-8. No embedded NULs.
- **.gitattributes**: Use to enforce line endings (`text eol=lf`).
- **Renormalize**: Use `git add --renormalize` for mass fixes.

## 10. The Commit Compass
...
- [012-core-pr-template-specific.md](./012-core-pr-template-specific.md) - PR template standards
- [035-core-workflow-slash-commands.md](./035-core-workflow-slash-commands.md) - Git workflow commands
