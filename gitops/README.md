...
이 표는 AppProject allow-list tightening 전에 현재 허용 항목의 근거를
고정한다. `apps` AppProject는 active workload kind와 onboarding-reserved
kind를 분리해 검증하고, `platform` AppProject는 Helm chart-managed 리소스
때문에 raw manifest scan만으로 축소하지 않는다.

| Project | Allow-list surface | Current allowed kinds | Evidence class | Tightening boundary | Validation |
| --- | --- | --- | --- | --- | --- |
| `apps` | `clusterResourceWhitelist` | `Namespace`. | Current `apps-generator -> apps` namespace creation fallback depends on `CreateNamespace=true`. | Remove only with the Namespace Ownership Matrix follow-up that reviews ApplicationSet reconciliation and app onboarding impact. | `bash scripts/validate-repo-quality-gates.sh .` and `bash scripts/validate-gitops-structure.sh`. |
...
| `apps` | `reserved namespaceResourceWhitelist` | `ConfigMap`, `DaemonSet`, `Deployment`, `ExternalSecret`, `HorizontalPodAutoscaler`, `NetworkPolicy`, `PodDisruptionBudget`, `Role`, `RoleBinding`, `Secret`, `ServiceAccount`, `StatefulSet`, `AnalysisRun`. | Reserved for documented app onboarding, secret-backed workloads, and common Kubernetes app shapes not active in `adminer`. | Remove only after a separate app onboarding model decision confirms the kind is unsupported. | `bash scripts/validate-repo-quality-gates.sh .` and app onboarding review. |
| `platform` | `platform AppProject allow-lists` | Raw platform manifests plus chart-managed platform components. | Platform root Applications include Helm charts and repo manifests; chart output can include resource kinds not visible in raw Git YAML. | Do not tighten from raw manifest scan alone; require chart render review and ArgoCD sync impact review. | `bash scripts/validate-repo-quality-gates.sh .`, `bash infrastructure/tests/verify-contracts-static.sh`, `bash scripts/validate-gitops-structure.sh`, and `bash scripts/validate-k8s-manifests.sh .`. |
...
- `apps` AppProject allow-list 최소화: 위 AppProject Allow-list Rationale
  Matrix가 active/reserved kind 기준을 검증한다. 실제 kind 제거는 별도 app
  onboarding model decision과 live reconciliation 영향 검토 후 진행한다.
