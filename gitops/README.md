...
| `platform/eso` | External Secrets Operator platform config owned by platform maintainers. | Kustomize config for Vault ClusterSecretStore and app ExternalSecret examples. | Depends on external Vault, exact `vault` audience, and approved Vault policy; secret values stay outside Git. | Validate with `python3 scripts/validate-vault-eso-contracts.py --root .` plus static secret checks; live readiness requires ESO/Vault checks. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `Vault API` | `vault-external.platform.svc.cluster.local`; EndpointSlice `172.18.0.8`. | `8200` | ClusterSecretStore path `secret`; platform paths `platform/argocd`, `platform/postgres-app`, `platform/notifications`. | Kubernetes auth role `eso-read-platform`, service account `external-secrets/external-secrets`, audience `vault`; no secret value is stored in Git. | Current in-cluster `http://vault-external.platform.svc.cluster.local:8200` URL is an annotated local-only exception and is not production TLS; external Vault management requires HTTPS and a verified CA. | External Vault operator owns `bound_audiences=vault`, Vault token, policy application, auth mount refresh, and secret rotation evidence. | Service contract lives in `platform`; ESO service account lives in `external-secrets`. | `python3 scripts/validate-vault-eso-contracts.py --root .`; `bash infrastructure/tests/verify-contracts-static.sh`; live ESO/Vault checks require trusted kubeconfig. |
...
| --- | --- | --- | --- | --- | --- |
| `ClusterSecretStore vault-backend` | External Secrets Operator uses Vault Kubernetes auth through `vault-backend`, role `eso-read-platform`, exact audience `vault`, mount path `kubernetes`, and KV path `secret`. | Cluster-scoped store name stays `vault-backend`; service account is `external-secrets` in namespace `external-secrets`; the HTTP boundary annotations remain exact. | Platform/security owns the ClusterSecretStore manifest; external Vault operator owns matching `bound_audiences=vault`, auth configuration, and policy application. | No Vault token, reviewer JWT, CA content, or secret value is committed; these values stay outside Git and live auth refresh is operator-approved. | `python3 scripts/validate-vault-eso-contracts.py --root .`; `bash infrastructure/tests/verify-contracts-static.sh`; `bash scripts/check-secret-handling.sh .`; live readiness uses `infrastructure/tests/verify-secrets.sh`. |
