...
이 디렉터리는 ArgoCD가 동기화할 GitOps 리소스를 포함한다. 프로젝트 경계(AppProject), ApplicationSet, 루트 애플리케이션, 플랫폼 리소스, 워크로드 리소스를 분리해 로컬 k3d 클러스터의 declarative desired state를 Git에 versioned 상태로 추적한다.
...
workload kind 정책을 정리한다. `repository-quality` 게이트는 active
...
| `platform-istio-cni`                 | `cni`              | `1.25.2`        | `istio-system`     | `ConfigMap`, `ServiceAccount`, `apps/DaemonSet`, `rbac.authorization.k8s.io/ClusterRole`, `rbac.authorization.k8s.io/ClusterRoleBinding`.                                                                                                                                                                                                                                                                                         |
...
| ------------------ | ---------------------------------------------------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Vault API`        | `openbao.hy.home.arpa` (OpenBao behind the external Traefik); CoreDNS custom zone resolves it to host `192.168.0.13` (ADR-0046). | `443`   | ClusterSecretStore path `secret`; platform paths platform/argocd, `platform/postgres-app`, `platform/notifications`. | Kubernetes auth role `eso-read-platform`, service account `external-secrets/external-secrets`, audiences `vault` and the API server audience (OpenBao sends the token to TokenReview as its own credential); no secret value is stored in Git. | ESO uses TLS to `https://openbao.hy.home.arpa` and verifies the certificate with the mkcert root CA ConfigMap `external-secrets/openbao-ca`; external Vault management also requires HTTPS and a verified CA. | External Vault operator owns `bound_audiences=vault`, Vault token, policy application, auth mount refresh, and secret rotation evidence. | Service contract lives in `platform`; ESO service account lives in `external-secrets`.                                        | `python3 scripts/validate-vault-eso-contracts.py --root .`; `bash scripts/validate-infrastructure-contracts.sh`; live ESO/Vault checks require trusted kubeconfig. |
| `PostgreSQL write` | `postgres-write-external.platform.svc.cluster.local`; EndpointSlice host `192.168.0.13` (`pg-router`, profile `postgres-ha`). | `15432` | Database name comes from Vault path `platform/postgres-app` property `db_name`.                                        | Kubernetes target secret `postgres-app-secret`; keys `db_name`, `username`, `password`.                                                            | Current repository contract is TCP only; TLS/CA material is owned by the external PostgreSQL service workspace if enabled.                                                                                 | External PostgreSQL owner rotates credentials in Vault; ESO refreshes the Kubernetes target secret on its configured interval.           | Service and target secret live in `platform`; app references must stay namespace-local or use approved app secret projection. | `bash scripts/validate-infrastructure-contracts.sh`; live reachability uses `infrastructure/verify/verify-external-services.sh`. <!-- pragma: allowlist secret -->  |
| `PostgreSQL read`  | `postgres-read-external.platform.svc.cluster.local`; EndpointSlice host `192.168.0.13` (`pg-router`, profile `postgres-ha`).  | `15433` | Database name comes from Vault path `platform/postgres-app` property `db_name`.                                        | Kubernetes target secret `postgres-app-secret`; keys `db_name`, `username`, `password`.                                                            | Current repository contract is TCP only; TLS/CA material is owned by the external PostgreSQL service workspace if enabled.                                                                                 | External PostgreSQL owner rotates credentials in Vault; ESO refreshes the Kubernetes target secret on its configured interval.           | Service and target secret live in `platform`; read/write split must remain explicit in service names.                         | `bash scripts/validate-infrastructure-contracts.sh`; live reachability uses `infrastructure/verify/verify-external-services.sh`. <!-- pragma: allowlist secret -->  |
| `Valkey auth`      | `valkey-external.platform.svc.cluster.local`; EndpointSlice host `192.168.0.13` (`mng-valkey`).   | `6379` → host `26379` | Vault path platform/argocd property `valkey_password`.                                                               | ExternalSecret `argocd-external-valkey` writes Kubernetes key `redis-password`.                                                                    | Current repository contract is TCP only; TLS/CA material is owned by the external Valkey service workspace if enabled.                                                                                     | External Valkey owner rotates `valkey_password` in Vault; ESO refreshes the ArgoCD target secret on its configured interval.             | Service contract lives in `platform`; ArgoCD target secret lives in `argocd`.                                                 | `bash scripts/validate-infrastructure-contracts.sh`; live secret readiness uses `infrastructure/verify/verify-secrets.sh`. <!-- pragma: allowlist secret -->        |
...
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ClusterSecretStore vault-backend`   | External Secrets Operator uses Vault Kubernetes auth through `vault-backend`, role `eso-read-platform`, exact audiences `vault` and `https://kubernetes.default.svc.cluster.local`, mount path `kubernetes`, and KV path `secret`. | Cluster-scoped store name stays `vault-backend`; service account is `external-secrets` in namespace `external-secrets`. | Platform/security owns the ClusterSecretStore manifest; external Vault operator owns matching `bound_audiences=vault`, auth configuration, and policy application. | No Vault token, reviewer JWT, CA content, or secret value is committed; these values stay outside Git and live auth refresh is operator-approved. | `python3 scripts/validate-vault-eso-contracts.py --root .`; `bash scripts/validate-infrastructure-contracts.sh`; `bash scripts/check-secret-handling.sh .`; live readiness uses `infrastructure/verify/verify-secrets.sh`. |
| `Platform postgres-app-secret`       | ExternalSecret reads Vault path `platform/postgres-app` properties `db_name`, `username`, and `password` through `vault-backend`; manifest removed until a workload consumes it (restore `gitops/platform/eso/postgres-app-secret.yaml` from Git history).                                              | Kubernetes target Secret name stays `postgres-app-secret` in namespace `platform`; `creationPolicy: Owner` remains explicit.                                        | Platform/security owns the ExternalSecret manifest; external PostgreSQL owner rotates source credentials in Vault.                                                 | Secret values remain outside Git; ESO refresh interval controls Kubernetes target Secret updates.                                                 | `bash scripts/validate-infrastructure-contracts.sh`; `bash scripts/check-secret-handling.sh .`; live readiness uses `infrastructure/verify/verify-secrets.sh`.                                                             |
...
- `apps` AppProject allow-list 최소화와 `CreateNamespace=true` 제거는
  desired-state에 반영되었고 위 matrix와 `repository-quality` 게이트가
...
- image tag와 workload-kind 정책 스캔: active `gitops/workloads/*`와 raw
  ...
  `repository-quality` 게이트에 포함한다. OPA/Conftest-style policy
...
- k8s router runtime proof: ingress-nginx LoadBalancer fallback과
  `CHECK_K8S_ROUTER=true` router 검증은 이 저장소 live test가 담당하지만,
  host 주소 `192.168.0.14` 할당과 이름 해석은 operator 경계에서 다룬다.
