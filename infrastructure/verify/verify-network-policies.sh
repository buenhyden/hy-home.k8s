...
[ "$argocd_np_ip" = "192.168.0.13/32" ] || fail "argocd valkey egress cidr mismatch (actual=$argocd_np_ip)"
...
# external-secrets namespace policy for OpenBao (external Traefik, ADR-0046)
...
[ "$eso_np_ip" = "192.168.0.13/32" ] || fail "external-secrets vault egress cidr mismatch (actual=$eso_np_ip)"
...
[ "$eso_np_port" = "443" ] || fail "external-secrets vault egress port mismatch (actual=$eso_np_port)"
...
[ "$kiali_prom_cidr" = "192.168.0.13/32" ] || fail "kiali prometheus egress cidr mismatch (actual=$kiali_prom_cidr)"
