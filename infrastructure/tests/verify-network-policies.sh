...
[ "$argocd_np_ip" = "172.30.0.12/32" ] || fail "argocd valkey egress cidr mismatch (actual=$argocd_np_ip)"
...
[ "$eso_np_ip" = "172.30.0.10/32" ] || fail "external-secrets vault egress cidr mismatch (actual=$eso_np_ip)"
